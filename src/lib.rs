...
impl Display for Вид_Словаря {
    fn fmt(&self, образ: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Вид_Словаря::запасник=> write!(образ, "Запасной"),
            Вид_Словаря::основной=>write!(образ,"Основной"),
        }
    }
}
#[derive(Debug, PartialEq, Clone, Copy)]
...
impl fmt::Display for Имена_страниц {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Имена_страниц::простое => write!(f, "Простые"),
            Имена_страниц::составное => write!(f, "Составные"),
...
pub struct Словарь_Переносов {
    ...
    pub однобуквенные: [Ячейка_замены; СЛОВАРЬ_ПЕРЕНОСОВ_ОДНОБУКВЕННЫЕ],   //одиночные слова
    pub двубуквенные: [Ячейка_замены; СЛОВАРЬ_ПЕРЕНОСОВ_ДВУБУКВЕННЫЕ],   //одиночные слова
    pub трехбуквенные: [Ячейка_замены; СЛОВАРЬ_ПЕРЕНОСОВ_ТРЕХБУКВЕННЫЕ], //одиночные слова
    pub многобуквенные: [Ячейка_замены; СЛОВАРЬ_ПЕРЕНОСОВ_МНОГОБУКВЕННЫЕ], //одиночные слова
    pub целиковые: [Ячейка_замены; СЛОВАРЬ_ПЕРЕНОСОВ_ЦЕЛИКОВЫЕ],     //одиночные слова
    pub исключения: [Ячейка_замены_с_исключением; СЛОВАРЬ_ПЕРЕНОСОВ_ИСКЛЮЧЕНИЯ],
    //
    //
}
//
...
//
pub static РАЗМЕР_РАЗДЕЛИТЕЛЕЙ:usize=135;
#[derive(Debug, Clone)]
...
impl Возможности_ячейки_замены_с_разделителями for Ячейка_замены_с_разделителями {
    fn добавить_re_исключения_изнутри(&self) -> Vec<Regex> {
        self.ряд_исключений
            .iter()
            .map(|ячейка| {
    ...
    fn добавить_оставшиеся_поля(&mut self) {
        ...
        self.re_образец_для_поиска=Regex::new(&format!(r#"\b{{start}}{}\w"#, self.искомое_слово)).unwrap();
        self.re_образец_для_замены=Regex::new(&format!(r#"\b{{start}}({})([\w]{{4,}})"#, self.искомое_слово)).unwrap();
    }
}
//словарь
#[derive(Debug, Clone)]
