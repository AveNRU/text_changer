...
pub fn считать_книги(
    ...
    let стопки_книг: Vec<lib::Книги>=
        ...
        //если архивный файл
            ...
            let книга_в_озу: Архив_в_озу =
            match zip_архив_в_память(&путь, HashMap::with_hasher(RandomState::default())) {
                Ok(успех) => успех,
                Err(ошибка) => {
            ...
            let содержимое: Vec<String> = read_utf8_без_переносов_строк(&путь); //чтение файла в UTF-8
            let стопка: Vec<lib::Вложения> = vec![lib::Вложения {
                содержимое: содержимое,
                имя: название_книги.clone(),
                имя_без_пути: re_получить_имя_файла_без_пути(
