...
pub fn считать_книги(
    ...
    use crate::utils::regex::{
        doc_docx, re_получить_имя_файла_без_пути, изображение_расширение_с_точкой,
        определить_имя_книги,
    };
    ...
    //получение книг
    let стопки_книг: Vec<Text_Changer::Книга_целиковая> = пути_до_книг
        .par_iter()
        .enumerate()
        .filter_map(|(_i, путь)| {
            ...
            //расширение файла
            let mut расширение_строка: String =
            ...
            //
            let расширение_самой_книги_подробно: Text_Changer::Основной_Вид_Расширения = определить_расширение_подробно(
                &расширение_строка.to_lowercase(),
                &mut куча_нераспознанных_расширений.lock().unwrap());
            if расширение_строка.len() > 6 && !sz_найти(&расширение_строка, "Без названия")
            {
                println!("Путь, где расширение больше 6 букв: {путь}");
                println!("Расширение больше 6 букв: {расширение_строка}");
            ...
            // //------------------------------------------------
            if расширение_самой_книги_подробно.fb2_mht() {
                // fb2_rtf_mht_mhtml(&путь) {
                // println!("вхождение: fb2_rtf_mht_mhtml");
                let содержимое: Vec<String> = {
                    if sz_найти(&расширение_строка, "mhtlml") {
                ...
                let стопка: Vec<Text_Changer::Вложения> = vec![Text_Changer::Вложения {
                    ...
                    расширение: Text_Changer::Расширение_полное{
                        строка:расширение_строка.clone(),
                        вид:расширение_самой_книги_подробно.clone(),
                        },
                }];
                ...
                Some(Text_Changer::Книга_целиковая {
                    вложения: стопка,
                    //содержимое:содержимое,
            ...
            //***********************************************************************************************************
            //если архивный файл
            else if расширение_самой_книги_подробно.архив_книга() {
            ...
            //перебор всех файлов архива
             книга_в_озу.into_par_iter().filter_map(|(имя_свитка, содержимое_архива)|  {
                 let итог_расширения=получить_подробное_расширение(&имя_свитка);
                //картинки не загонять в utf8
                if итог_расширения.вид.мусорное_расширение()//изображение_расширение_с_точкой(&имя_свитка) || мусорное_содержимое_архивов(&имя_свитка) ||нет_разрешения (&имя_свитка)
                {
                       //println!("мусор вложение - |{}| ",имя_свитка);
                     //println!("Считывание  1 |{}|",имя_свитка);
                    //println!("имя_свитка |{}| содержимое_в_байтах |{}|",имя_свитка,содержимое_архива.len());
                    Some(Text_Changer::Вложения {
                        содержимое: Vec::new(), //пустота - так это рисунок, нельзя читать
                        имя_без_пути:
                            re_получить_имя_файла_без_пути(&имя_свитка),
                        имя: имя_свитка,
                        содержимое_в_байтах: содержимое_архива.clone(),
                        кодировка:Text_Changer::Кодировка::Не_определено,
                        расширение: Text_Changer::Расширение_полное{
                            строка:итог_расширения.строка,
                            вид:итог_расширения.вид,
                            },
                    })
                }
                //если это не изображение и  не шриафты
                else {
                    let содержимое_файла: Vec<String> =read_utf8_из_ряда_u8(&содержимое_архива, &имя_свитка);
                    //если это htm страница
                    if итог_расширения.вид.htm_html_xhtml_ncx() {
                       // println!("Считывание  2 |{}|",имя_свитка);
                        let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                        let новое_содержимое_файла: Vec<String> =htm_utf8_без_переносов_строк(&содержимое_файла,&имя_свитка,&расширение_самой_книги_подробно);
                        let кодировка: Text_Changer::Кодировка=определить_кодировку(&новое_содержимое_файла,&имя_свитка,&расширение_самой_книги_подробно,&путь,&mut сообщения_свои.кодировка);
                       //
                        сообщения
                            .lock()
                            .unwrap()
                   ...
                   // println!("htm вложение - |{}| ",имя_свитка);
                    ...
                    //если это не htm содержимое
                         ...
                         //  println!("htm остальное - |{}| ",имя_свитка);
                    //     println!("Считывание  3 |{}| расширения |{}|-|{}|",имя_свитка,итог_расширения.0,итог_расширения.1);
                        let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                        let кодировка: Text_Changer::Кодировка=определить_кодировку(&содержимое_файла,&имя_свитка,&расширение_самой_книги_подробно,&путь,&mut сообщения_свои.кодировка);
                        //
                        сообщения
                            .lock()
                            .unwrap()
           ...
           //----------------------------------------------------------------
            ...
            //------------------------------------------------
            else if расширение_самой_книги_подробно.md_yml_fs() {
                let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                ...
                let кодировка: Text_Changer::Кодировка =
                    определить_кодировку(&содержимое,&название_книги,&расширение_самой_книги_подробно,&путь,&mut сообщения_свои.кодировка);
                let стопка: Vec<Text_Changer::Вложения> = vec![Text_Changer::Вложения {
                    содержимое: содержимое,
                    имя: название_книги.clone(),
                    имя_без_пути: re_получить_имя_файла_без_пути(
            ...
            //------------------------------------------------
            else if расширение_самой_книги_подробно.htm_html_xhtml_ncx() {
                let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                //      println!("вхождение: htm_html_xhtml");
                let содержимое: Vec<String> =
                    считать_в_utf8_без_переносов_строк_htm_не_архив(&путь, &название_книги); //чтение файла в UTF-8

                /*for строка_книги in содержимое.iter() {
                    println!("|{}|",строка_книги);
                }*/
                let кодировка: Text_Changer::Кодировка =
                    определить_кодировку(&содержимое,&название_книги,&расширение_самой_книги_подробно,&путь,&mut сообщения_свои.кодировка);
                //println!("Книга |{}| кодировка |{}|",название_книги,кодировка);
                let стопка: Vec<Text_Changer::Вложения> = vec![Text_Changer::Вложения {
                    содержимое: содержимое,
                    имя: название_книги.clone(),
                    имя_без_пути: re_получить_имя_файла_без_пути(
            ...
            // если вложения - не книга
                    ...
                    || !sz_найти(&имя_файла_без_пути, ".")
                {
                    ...
                    сообщения
                        ...
                        .extend(сообщения_свои.кодировка);
                    //
                    println!("Не книга: |{}| название |{}|-расширение |{}|",путь,название_книги,расширение_самой_книги_подробно);
                    Some(Text_Changer::Книга_целиковая {
                        вложения: стопка,
                        //содержимое:содержимое,
                        путь: путь.clone(),             //путь полный
                        название_книги: название_книги, //имя книги
                        расширение:Text_Changer::Расширение_полное { строка: расширение_строка.clone(), вид: расширение_самой_книги_подробно.clone() },
                        книга_ли: false,
                        архив: Default::default(),
    ...
    *сообщения_приход = сообщения;
    ...
    return Text_Changer::Книги_в_ОЗУ {
        книги: стопки_книг,
        размер_в_озу: количество_мегабайт,
    };
}
pub fn определить_расширение_подробно(
    ...
    match расширение {
        ...
        "opf"=>Text_Changer::Основной_Вид_Расширения::Разметка_Паутины(Text_Changer::Вид_Разметки_Паутины::Opf),
        "xhtml"=> Text_Changer::Основной_Вид_Расширения::Разметка_Паутины(Text_Changer::Вид_Разметки_Паутины::Xhtml),
        "ttf"=> Text_Changer::Основной_Вид_Расширения::Шрифты(Text_Changer::Вид_Шрифтов::Ttf),
         "otf"=> Text_Changer::Основной_Вид_Расширения::Шрифты(Text_Changer::Вид_Шрифтов::Otf),
        ...
        "log" => Text_Changer::Основной_Вид_Расширения::Простая_письменность(Text_Changer::Вид_простой_письменности::Log),
        ...
        "tiff" => Text_Changer::Основной_Вид_Расширения::Изображение(Text_Changer::Вид_Изображения::Tiff),
        "jpe" => Text_Changer::Основной_Вид_Расширения::Изображение(Text_Changer::Вид_Изображения::Jpe),
        ...
        "css" => Text_Changer::Основной_Вид_Расширения::Мусорная_Разметка(Text_Changer::Вид_Мусорные_Разметки_Паутины::Css),
        ...
        "thmx" => Text_Changer::Основной_Вид_Расширения::Мусорная_Разметка(Text_Changer::Вид_Мусорные_Разметки_Паутины::Thmx),
        ...
        "pdf"=> Text_Changer::Основной_Вид_Расширения::Книга(Text_Changer::Вид_Книги::Одиночная(Text_Changer::Вид_одичноной_книги::Pdf)),
        "ru" => Text_Changer::Основной_Вид_Расширения::Реклама_html(Text_Changer::Вид_рекламы_HTML::Ru),


        _ => {
            println!("Не определено расширение |{}|",расширение);
            panic!();
...
pub fn получить_подробное_расширение(
    имя: &String,
) -> Text_Changer::Подробный_вид_расширения_с_кучей {
    let mut куча_нераспознанных_расширений: rapidhash::fast::RapidHashSet<String> =
        rapidhash::fast::RapidHashSet::default();
    let расширение: String = match re_получить_строку_с_описанием(
        &имя,
        &RE_РАСШИРЕНИЕ_ФАЙЛА_C_ТОЧКОЙ,
        "Не удалось выдрать расширение файла с точкой",
    ) {
    ...
    let расширение_подробно: Text_Changer::Основной_Вид_Расширения =
        определить_расширение_подробно(
            &расширение.to_lowercase(),
            &mut куча_нераспознанных_расширений,
        );
    //
    return Text_Changer::Подробный_вид_расширения_с_кучей {
        строка: расширение,
        вид: расширение_подробно,
        куча: куча_нераспознанных_расширений,
    };
}
