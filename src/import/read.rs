...
pub fn считать_книги(
    ...
    let стопки_книг: Vec<Text_Changer::Книги> = пути_до_книг
        ...
        .filter_map(|(_i, путь)| {
            ...
            // //------------------------------------------------
            if расширение_подробно.fb2_mht() {
                ...
                let стопка: Vec<Text_Changer::Вложения> = vec![Text_Changer::Вложения {
                    ...
                    расширение_подробно:расширение_подробно.clone(),
            ...
            //***********************************************************************************************************
            //если архивный файл
            else if расширение_подробно.архив_книга() {
             ...
             книга_в_озу.into_par_iter().filter_map(|(имя_свитка, содержимое_архива)|  {
                ...
                //если это не изображение и  не шриафты
                    ...
                    if итог_расширения.1.htm_html_xhtml() {
                        let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                        let новое_содержимое_файла: Vec<String> =htm_utf8_без_переносов_строк(&содержимое_файла,&имя_свитка);
                        let кодировка: Text_Changer::Кодировка=определить_кодировку(&новое_содержимое_файла,&имя_свитка,&расширение_подробно,&путь,&mut сообщения_свои.кодировка);
                       //
                        сообщения
                            .lock()
                            .unwrap()
                    ...
                    //если это не htm содержимое
                        ...
                        let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                        let кодировка: Text_Changer::Кодировка=определить_кодировку(&содержимое_файла,&имя_свитка,&расширение_подробно,&путь,&mut сообщения_свои.кодировка);
                        //
                        сообщения
                            .lock()
                            .unwrap()
           ...
           //----------------------------------------------------------------
            ...
            //------------------------------------------------
            else if расширение_подробно.md_yml_fs() {
                let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                ...
                let кодировка: Text_Changer::Кодировка =
                    определить_кодировку(&содержимое,&название_книги,&расширение_подробно,&путь,&mut сообщения_свои.кодировка);
                let стопка: Vec<Text_Changer::Вложения> = vec![Text_Changer::Вложения {
                    содержимое: содержимое,
                    имя: название_книги.clone(),
                    имя_без_пути: re_получить_имя_файла_без_пути(
            ...
            //------------------------------------------------
            else if расширение_подробно.htm_html_xhtml() {
                let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
