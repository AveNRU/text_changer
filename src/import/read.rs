...
pub fn считать_книги(
    ...
    образцы_для_переноса_строк_повторяются_ли();
    //основной путь
    let пути_общие: lib::Пути_Общие = Default::default();
             ...
             книга_в_озу.into_par_iter().filter_map(|(имя, содержимое_архива)|  {
                ...
                //если это не изображение и  не шриафты
                    ...
                    if htm_html_xhtml(&имя) {
                        ...
                        let кодировка: lib::Кодировка=определить_кодировку(&новое_содержимое_файла);
                       // println!("содержимое xhtml: {}",re_получить_имя_файла_без_пути(&имя));
                        ...
                        let кодировка: lib::Кодировка=определить_кодировку(&содержимое_файла);
                        Some(lib::Вложения {
...
fn образцы_для_переноса_строк_повторяются_ли() {

        let образцы:lib::Образцы_для_разбиения_html=Default::default();
