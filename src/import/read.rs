...
pub fn считать_книги(
    ...
    use crate::utils::regex::{
        doc_docx, fb2_rtf_mhtml, fb3_epub, md_fs_yml, изображение_расширение_с_точкой, определить_имя_книги,изображение_расширение_без_точки
    };
    use crate::utils::zip::{VirtualFs, zip_архив_в_память};
    use std::default::Default;
    //основной путь
    let пути_общие: lib::Пути_Общие = Default::default();
    let mut содержимое_папки = lib::Содержимое_папок::default();
    //получение значение корневого доступа к скрипту (где он лежит, как решила ОС)
        ...
        //если архивный файл
           ...
           // let mut указатель_изображения:usize=0;
            for (имя, содержимое_архива) in книга_в_озу.into_iter() {
                ...
                //println!("Считывание файла: {}",имя);
                if изображение_расширение_с_точкой(&имя) {
                   // println!("Считывание изображения: {}",имя);
                    ...
                    //проверочный вывод изображения
                    //let путь2=&format!("изображение_{}",указатель_изображения);
                   // let mut вывод = match File::create(&путь2) {
                  //      Ok(сообщение) => {println!("{:?}",сообщение);сообщение},
                  //      Err(e) => panic!(
                 //           "Не удалось создать файл книги: {} по причине: {}",
                  //          путь2, e
                  //      ),
                  //  };
                  //  вывод.write_all(&содержимое_архива).unwrap();
                   // указатель_изображения+=1;
                }
