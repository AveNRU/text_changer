...
pub fn считать_книги(
    ...
    use crate::utils::regex::{
        ...
        мусорное_содержимое_архивов, нет_разрешения, определить_имя_книги,
    };
        ...
        .filter_map(|(_i, путь)| {
            ...
            //перебор всех файлов архива
             книга_в_озу.into_par_iter().filter_map(|(имя_свитка, содержимое_архива)|  {
                ...
                if итог_расширения.1.мусорное_расширение()//изображение_расширение_с_точкой(&имя_свитка) || мусорное_содержимое_архивов(&имя_свитка) ||нет_разрешения (&имя_свитка)
                {
                    ...
                    Some(Text_Changer::Вложения {
                        ...
                        расширение_подробно:итог_расширения.1,
                        расширение:итог_расширения.0,
                    })
                }
                //если это не изображение и  не шриафты
                    ...
                    if итог_расширения.1.htm_html_xhtml_ncx() {
                   ...
                   // println!("htm вложение - |{}| ",имя_свитка);
                       // println!("содержимое xhtml: {}",re_получить_имя_файла_без_пути(&имя));
                        Some(Text_Changer::Вложения {
                            ...
                            содержимое_в_байтах: содержимое_архива.clone(),
                            кодировка,
                            расширение_подробно: итог_расширения.1.clone(),
                             расширение:итог_расширения.0.clone(),
                        })
                    }
...
pub fn получить_подробное_расширение(
    ...
    return (
        расширение,
        расширение_подробно,
        куча_нераспознанных_расширений,
    );
}
