...
pub fn считать_книги(
    ...
    use crate::utils::regex::{
        doc_docx, md_fs_yml, re_получить_имя_файла_без_пути, изображение_расширение_с_точкой,
        ...
        .filter_map(|(_i, путь)| {
            ...
            //перебор всех файлов архива
             книга_в_озу.into_par_iter().filter_map(|(имя, содержимое_архива)|  {
                 let итог_расширения=получить_подробное_расширение(&имя);
                ...
                if изображение_расширение_с_точкой(&имя) || мусорное_содержимое_архивов(&имя)
                {
                    Some(Text_Changer::Вложения {
                        ...
                        расширение_подробно:итог_расширения.0,
                    })
                }
                //если это не изображение и  не шриафты
                    ...
                    if итог_расширения.0.htm_html_xhtml() {
                        let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                        let новое_содержимое_файла: Vec<String> =htm_utf8_без_переносов_строк(&содержимое_файла,&имя);
                        let кодировка: Text_Changer::Кодировка=определить_кодировку(&новое_содержимое_файла,&имя,&расширение_подробно,&путь,&mut сообщения_свои.кодировка);
                       //
                        сообщения
                            .lock()
                            .unwrap()
                       ...
                       // println!("содержимое xhtml: {}",re_получить_имя_файла_без_пути(&имя));
                        Some(Text_Changer::Вложения {
                            ...
                            содержимое_в_байтах: содержимое_архива.clone(),
                            кодировка,
                            расширение_подробно: итог_расширения.0.clone(),
                        })
                    }
                    //если это не htm содержимое
                    else {
                        let mut сообщения_свои: Text_Changer::Сообщения = Text_Changer::Сообщения::default();
                        let кодировка: Text_Changer::Кодировка=определить_кодировку(&содержимое_файла,&имя,&расширение_подробно,&путь,&mut сообщения_свои.кодировка);
                        //
                        сообщения
                            .lock()
                            .unwrap()
...
pub fn получить_подробное_расширение(
    ...
    //
    return (расширение_подробно, куча_нераспознанных_расширений);
}
