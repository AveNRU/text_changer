                ...
                словарь_разделителей.ряд_1.iter().enumerate()
            {
                ...
                //
                    let количество_совпадений: usize =
                        ячейка.re_образец_для_замены.find_iter(&строка.clone()).count();
                ...
                //
                    ...
                    for указатель_захода in 0..количество_совпадений {
                       ...
                       // let число_захода:usize=указатель_захода;
                        ...
                        if есть_ли_исключение(&ячейка.ряд_re_исключений,&все_совпадения[число_захода].as_str()) {continue}
...
pub fn создать_словарь_разделителей() -> Словарь_разделителей {
    ...
    let mut ряд_1: Словарь_разделителей = Словарь_разделителей {
        ряд_1: [
                ...
                замена: "волно-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}волно(\w){4,}")
                    .unwrap(),
                re_образец_для_замены: Regex::new(r"\b{start}(волно)([\w]{4,})")
                    .unwrap(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
            ...
            Ячейка_замены_с_разделителями {
                ...
                re_образец_для_замены: Regex::new(r"\b{start}(пере)([\w]{4,})")
                    .unwrap(),
                ряд_re_исключений:vec![Regex::new(r"\b{start}(передне)")
                    .unwrap(),],
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
...
pub fn есть_ли_исключение (
    ...
    for исключение_ряда in ряд_re_исключений.iter()
    {
        ...
        //проверка
        if исключение_ряда
            .is_match(&совпадение_re_в_строке)
        {
           return true
        }
    }
