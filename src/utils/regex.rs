    ...
    содержимое
        ...
        .for_each(|(указатель_строки, строка)| {
                ...
                словарь_разделителей.ряд_1.iter().enumerate()
            {
                ...
                //
                    let количество_совпадений: usize =
                        ячейка.re_образец_для_замены.find_iter(&строка).count();
                //если не нашло - то следующее слово
                if количество_совпадений==0 {continue 'круговорот_главный}
                //
                    if количество_совпадений > 1 {
                        условие_вывода_1 = true;
                    }
                    /*let строка_2:String=строка.to_string();
                    let выдрано_1:Captures=match ячейка.re_образец_для_замены.captures(&строка_2) {
                        Some(выдрано)=>выдрано,
                        None=>continue 'круговорот_главныq,
                    };*/
                    ...
                    let все_совпадения_в_ряд: Vec<String> = все_совпадения
                        .iter()
                        .map(|совпадение_re| совпадение_re.as_str().to_string())
                        .collect();
                    /*if условие_вывода_1 &&указатель_строки==331  {
                        println!("Общий вывод: {:?}",все_совпадения);
                        println!("Количество совпадений: {количество_совпадений}");
                    }*/
                    //println!("строка№: |{указатель_строки}|, Совпадений: {количество_совпадений}, Выдер: {}, итог {:?}\r\n",ячейка.re_образец_для_замены,ячейка.re_образец_для_замены.captures(&строка));
                    //      перебор количества совпадений
                    //'круговорот_совпадений: for (совпадение_числитель,совпадение_re) in все_совпадения.iter().enumerate() {
                    ...
                    {
                        /*let выдрано:Captures= match ячейка.re_образец_для_замены.captures(&строка) {
                            Some(успех)=>успех,
                            None=>
                                {
                                    println!("2-Не нашло совпадений: строка: |{указатель_строки}|,  замена: |{}|",ячейка.re_образец_для_замены);
                                    continue 'круговорот_совпадений
                                },
                        };*/
                        //проверка на исключения
                        /* if условие_вывода_2 {
                            println!("количество исключений для |{}| всего: |{}|", ячейка.re_образец_для_замены, ячейка.ряд_re_исключений.len());
                        }*/
                        for исключение_ряда in ячейка.ряд_re_исключений.iter()
                        {
                            //если исключение - то следующее совпадение этого образца
                            //if выдрано[совпадение_числитель].
                            /* let проверка = match все_совпадения.get(совпадение_числитель) {
                                Some(успех) => {
                                    успех
                                    //  println!("Нашло исключение: {}",&выдрано[совпадение_числитель]);continue 'круговорот_совпадений
                                },
                                None => {
                                    println!("1-Не нашло указатель в совпадениях: |{совпадение_числитель}| из : {:?}", все_совпадения);
                                    continue 'круговорот_совпадений
                                }
                            };*/

                            //проверка
                            /*if условие_вывода_1 && условие_вывода_2 {
                                println!("1!: Выдрано |{:?}|, числитель: |{совпадение_числитель}| образец: {:?}", все_совпадения, ячейка.re_образец_для_замены);
                            }*/
                            if исключение_ряда
                                .is_match(&все_совпадения[совпадение_числитель].as_str())
                            {
                                /*  println!(
                                    "Нашло исключение: {}",
                                    &все_совпадения[совпадение_числитель].as_str()
                                );*/
                                continue 'круговорот_совпадений;
                            }
                        }
                        ...
                        /*let проверка = match все_совпадения.get(совпадение_числитель) {
                            Some(успех) => { успех },
                            None => {
                                println!("Не нашло совпадение: |{совпадение_числитель}| из : {:?}", все_совпадения);
                                continue 'круговорот_совпадений
                            }
                        };*/
                        //
                        let строка2: String = строка.to_string();
                        let все_совпадения: Vec<Match> =
                            ячейка.re_образец_для_замены.find_iter(&строка2).collect();
                            ...
                            все_совпадения.iter().enumerate()
                        {
                            ...
                            //вынимаем
                            let captures: Captures = ячейка
                                .re_образец_для_замены
                                .captures(&_само_совпадение_re.as_str())
                                .unwrap();
                            let замена: String = format!("{}-{}", &captures[1], &captures[2]);
                            //
                            let mut замененная_строка: String = строка.clone();
                            //
                            // Проверяем, что индексы в пределах строки
                            //  println!("строка: |{}|, замена: |{замена}| само совпадение re: {:?}", указатель_строки, _само_совпадение_re);
                            //
                            // Получаем корректные границы символов
                            let начало = _само_совпадение_re.start();
                            let конец = _само_совпадение_re.end();
                            // Корректируем индексы до границ символов
                            let начало = замененная_строка.floor_char_boundary(начало);
                            let конец = замененная_строка.floor_char_boundary(конец);
                            //
                            замененная_строка.replace_range(начало..конец, &замена);
                            /*  замененная_строка.replace_range(
                                _само_совпадение_re.start().._само_совпадение_re.end(),
                                &замена
                            );*/
    ...
    let mut ряд_1: Словарь_разделителей = Словарь_разделителей {
        ряд_1: [
            ...
            Ячейка_замены_с_разделителями {
                ...
                re_образец_для_поиска: Regex::new(r"\b{start}все(\w){4,}")
                    .unwrap(),
            ...
            Ячейка_замены_с_разделителями {
                ...
                re_образец_для_замены: Regex::new(r"\b{start}(пре)([\w]{4,})")
                    .unwrap(),
                ряд_исключений: vec!["пред".to_string()],
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
            ...
            Ячейка_замены_с_разделителями {
                ...
                re_образец_для_поиска: Regex::new(r"\b{start}лже(\w){3,}")
                    .unwrap(),
