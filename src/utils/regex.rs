...
pub fn замена_слов_через_кучу(
    ...
    _сообщение: &str,
    _расширение: &Text_Changer::Расширение_полное,
    куча_пропусков: &rapidhash::fast::RapidHashSet<usize>,
                    ...
                    for указатель_образца in куча_указателей.iter() {
                        ...
                        if куча_указателей.len() > 2 {
                            ...
                            if sz_найти(&строка, &словарь[*указатель_образца].искомое_слово)
                            {
                                ...
                                {
                                    if данные_при_загрузке.вывод_всех_словарей
                                    {
...
//многопоточность
pub fn добавить_разделители_в_книгу(
    ...
    данные_при_загрузке: &crate::ui_gpui::gpui_main::Данные_при_загрузке,
) {
    if !данные_при_загрузке.включить_разделители {
        //println!("Разделители отключен");
        return;
    }
    use Text_Changer::Образцы_Исключений_Разделителей;
                    ...
                    if ячейка.re_образец_для_замены.is_match(&строка) {
                        ...
                        let заход:usize=заход_главный-количество_успехов;
                                ...
                                .captures(&все_совпадения[заход].as_str())
                            {
                                ...
                                //устанавливаем указатель на исключения
                                let указатель_на_re_исключения:Образцы_Исключений_Разделителей =
                                    match уровень_букв_в_замене
                                     {
                                        Text_Changer::Правописание_слова::Все_строчные =>
                                        //
                                        match уровень_букв_в_словаре {
                                           Text_Changer::Правописание_слова::Все_строчные=>  Text_Changer::Образцы_Исключений_Разделителей{
                                               ряд_re_пропуски:&словарь_разделителей_полный[указатель_словаря_переносов].содержимое
                                               [указатель_ячейки]
                                               .ряд_re_пропуски,
                                               ряд_re_обязательства:&словарь_разделителей_полный[указатель_словаря_переносов].содержимое
                                               [указатель_ячейки]
                                               .ряд_re_обязательства},
                                           //
                                           Text_Changer::Правописание_слова::С_Заглавной|Text_Changer::Правописание_слова::Все_Заглавные=>
                                           Text_Changer::Образцы_Исключений_Разделителей{
                                               ряд_re_пропуски:&словарь_разделителей_полный[1].содержимое
                                               [указатель_ячейки]
                                               .ряд_re_пропуски,
                                               ряд_re_обязательства:&словарь_разделителей_полный[1].содержимое
                                               [указатель_ячейки]
                                               .ряд_re_обязательства,
                                           },
                                           _=>panic!(),
                                        ...
                                        //если в замене заглавные буквы - то всегда все заглавные выставлять
                                         Text_Changer::Правописание_слова::Все_Заглавные =>
                                            Text_Changer::Образцы_Исключений_Разделителей{
                                                ряд_re_пропуски:&словарь_разделителей_полный[2].содержимое
                                                [указатель_ячейки]
                                                .ряд_re_пропуски,
                                                ряд_re_обязательства:&словарь_разделителей_полный[2].содержимое
                                                [указатель_ячейки]
                                                .ряд_re_обязательства,
                                            },
                                            _=>panic!()
                                ...
                                //
                                if есть_ли_исключения_и_обязательства(
...
pub fn создать_словарь_разделителей(
    ...
    let mut итоговый_словарь: Словарь_разделителей = Словарь_разделителей {
        содержимое: vec![
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["тонкост".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["недолг".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["близки".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["потомк".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["мозгово".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                .в_умные(),
                ряд_обязательств: vec!["недолг".to_string(), "неколеб".to_string()]
                    .в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["левон".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["лёгкост".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["слабост".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["солнцевск".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec![
                    ...
                    "прежде".to_string(),
                    "прещ".to_string(),
                    "прет".to_string(), // убрал - претворив исключение
                    "предмет".to_string(),
                    "пред".to_string(),
                ]
                .в_умные(),
            ...
            Ячейка_замены_с_разделителями {
                ...
                .в_умные(),
                ряд_обязательств: vec!["тепловоз".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["молодост".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["злобн".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["долговы".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["умолч".to_string(), "умолк".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["крышеват".to_string(), "крышевал".to_string()]
                    .в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["верховн".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_пропусков: vec!["вышен".to_string(), "вышения".to_string()]
                    .в_умные(),
                ряд_обязательств: vec!["вышеназван".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_обязательств: vec!["военн".to_string()].в_умные(),
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
                искомое_слово:
    ...
    //
    //
    проверка_ряда_разделителей_на_повторы(
        &итоговый_словарь.содержимое,
        Вид_Слов_Разделителей::Пропуски,
    )
    .unwrap();
    //
    проверка_ряда_разделителей_на_повторы(
...
pub fn создать_словарь_переносов() -> Словарь_Переносов {
    //use crate::dictionary_0::проверка_ряда_regex;
...
use Text_Changer::{
    Вид_Слов_Разделителей, Умная_Строка, Ячейка_замены_с_разделителями
};
//
pub fn проверка_ряда_разделителей_на_повторы(
    словарь: &Vec<Ячейка_замены_с_разделителями>,
    вид_слов: Вид_Слов_Разделителей,
) -> Result<(), ()> {
    //
    for ячейка_замены in словарь.iter() {
        //
        let mut куча_проверочная: rapidhash::fast::RapidHashSet<String> =
            rapidhash::fast::RapidHashSet::default();
        //
        let ссылка_на_раздел: &Vec<Умная_Строка> = match вид_слов {
            Вид_Слов_Разделителей::Обязательства => {
                &ячейка_замены.ряд_обязательств
            }
            //
            Вид_Слов_Разделителей::Пропуски => {
                &ячейка_замены.ряд_пропусков
            }
        };
        //заполнение оставшихся полей
        for образец in ссылка_на_раздел.iter() {
            ...
            if !куча_проверочная.contains(&образец.получить_значение())
            {
                ...
                println!(
                    "Разделители - |{}| (полное схождение) |{}| содержит повторно образец |{}|",
                    вид_слов, ячейка_замены.искомое_слово, образец
                );
            }
        }
        //собрать ряд RE исключений
    ...
    for ячейка_замены in словарь.iter() {
        ...
        //заполнение оставшихся полей
        for (указатель_1, образец_1) in ссылка_на_раздел.iter().enumerate()
        {
            for (указатель_2, образец_2) in ссылка_на_раздел
                .iter()
                .enumerate()
...
use crate::import::functions::преобразовать_слово_с_чертой_в_конце_в_умную_строку;
use crate::xlsx::import_xlsx::{
    найти_особые_знаки_в_умной_строке, обратно_убрать_особые_знаки_умная_строка,
};
...
pub fn создать_счётчики_словаря(
    ...
    //если не надо выводить словари - то и счётчики не надо создавать
    if !данные_при_загрузке.вывод_всех_словарей {
        return return Arc::new(Default::default());
    }
    //
...
pub fn есть_ли_исключение(
    образцы: &[Regex], срез_строки: &str
) -> bool {
    образцы
        .par_iter()
        .any(|шаблон| шаблон.is_match(срез_строки))
}
use Text_Changer::Образцы_Исключений_Разделителей;
pub fn есть_ли_исключения_и_обязательства(
    образцы: Образцы_Исключений_Разделителей,
    срез_строки: &str,
) -> bool {
    if образцы
        .ряд_re_пропуски
        .par_iter()
        .any(|шаблон| шаблон.is_match(срез_строки))
    //если нашлось исключение - то проверить есть ли обязательные слова , когда нужно поставить разделитель
    {
        !образцы
            .ряд_re_обязательства
            .par_iter()
    ...
    //если не нашлось исключение - то вернуть ложь
    else {
        return false;
    }
}
...
fn создать_разделы_словаря_переносов() -> Словарь_Переносов {
    ...
    const ТРЕХБУКВЕННЫЕ_РЯД: [&'static str;
        ...
        "-лов", "-кое", "-сия", "-сов", "-тит", "-тей", "-нем", "-век", "-гие", "-нии", "-зал",
        "-бая", "-кий", "-пии", "-мая", "-дью", "-ней", "-тие", "-мом", "-цев", "-цию", "-щей",
        "-гии", "-щих", "-нее", "-зуя",
    ];
    const ДВУБУКВЕННЫЕ_РЯД: [&'static str;
        ...
        "-ча", "-пи", "-па", "-во", "-ке", "-ни", "-лю", "-ре", "-ши", "-по", "-ря",
    ];
    ...
    let исключения: [Ячейка_замены_с_исключением;
        ...
        Ячейка_замены_с_исключением {
            искомое_слово: "-ном".to_string(),
            замена: "ном".to_string(),
            re_образец_для_поиска: Regex::new(r"(?i)-ном\b{end}").unwrap(),
            re_исключение: vec![Regex::new(r"(?i)\d-ном\b{end}").unwrap()],
        },
        Ячейка_замены_с_исключением {
