...
pub fn замена_слов_через_кучу(
   ...
   // содержимое
    //    .par_iter_mut()
     ...
     //   .for_each(|(указатель, строка)| {
            ...
            for (образец, куча_указателей) in словарь_куча.iter() {
                ...
                if sz_найти(&содержимое[указатель], &образец) {
                    ...
                    for указатель_образца in куча_указателей.iter() {
                        ...
                        if куча_указателей.len() > 2 {
                            ...
                            if sz_найти(&содержимое[указатель], &искомое_слово[*указатель_образца])
                            {
                                let замененная_строка = &re_образцы
                                    [*указатель_образца]
                                    .replace_all(&содержимое[указатель], &замены[*указатель_образца]);
                                //
                                let замененная_строка = замененная_строка.to_string();
                                if замененная_строка.as_str() != содержимое[указатель].as_str()
                                {
                                ...
                                //        .fetch_add(1, Ordering::Relaxed);
                                    содержимое[указатель] = замененная_строка;
                                }
                                // Заменяем строку
                               // содержимое[указатель] = замененная_строка;
                        ...
                        //если 1-2 значения в ключе
                            ...
                            let замененная_строка = &re_образцы[*указатель_образца]
                                .replace_all(&содержимое[указатель], &замены[*указатель_образца]);
    ...
    for (i, атомарный) in атомарные_счетчики.iter().enumerate() {
        ...
        if счётчик_словаря_внутренний[i]!=неделимый {
            println!("Слово: {}, не равно количество счётчиков. atom: {}, mutex: {}",
                     искомое_слово[i],
                     неделимый,
                     счётчик_словаря_внутренний[i],
            );
        }
