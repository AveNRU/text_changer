...
//если это картинка
use crate::lib::{self, Быстрый_Словарь, Полный_Словарь, Словарь_разделителей, Счётчик_замен, Счётчик_разделителей, Ячейка_словаря, СЛОВАРЬ_ПЕРЕНОСОВ_ОДНОБУКВЕННЫЕ};
...
pub fn создать_словарь_разделителей() -> Словарь_разделителей {
    ...
    let mut ряд_1: Словарь_разделителей = Словарь_разделителей {
        ряд_1: [
            ...
            Ячейка_замены_с_разделителями {
                ...
                ряд_исключений: vec!["подобн".to_string(),"подобен".to_string(),"подорв".to_string(),],
                ..Default::default()
            },
            Ячейка_замены_с_разделителями {
    ...
    for ячейка_замены in ряд_1.ряд_1.iter_mut() {
        ...
        ячейка_замены.ряд_исключений=ячейка_замены.ряд_исключений
            .iter()
    ...
    //
    //проверка образцов
    проверка_ряда_regex_разделителей(образцы_поиска_re_для_проверки,
...
pub fn создать_словарь_замен() -> Словарь_Переносов {
    use crate::dictionary_0::проверка_ряда_regex;
    let словарь_замен: Словарь_Переносов = создать_разделы_словаря_переносов();
    //
    //let словарь_второй
...
fn создать_разделы_словаря_переносов() -> Словарь_Переносов {
    ...
    let однобуквенные_ряд:[String;lib::СЛОВАРЬ_ПЕРЕНОСОВ_ОДНОБУКВЕННЫЕ] =[
        "-о".to_string(),
        "-а".to_string(),
        "-ь".to_string(),
        "-ы".to_string(),
    ...
    let многобуквенные_ряд:[String;lib::СЛОВАРЬ_ПЕРЕНОСОВ_МНОГОБУКВЕННЫЕ]= [
        "-ройства ".to_string(),
    ...
    let трехбуквенные_ряд:[String;lib::СЛОВАРЬ_ПЕРЕНОСОВ_ТРЕХБУКВЕННЫЕ]= [
        "-ков".to_string(),
        "-щий".to_string(),
        "-дят".to_string(),
        "-ter".to_string(),
    ...
    let двубуквенные_ряд:[String;lib::СЛОВАРЬ_ПЕРЕНОСОВ_ДВУБУКВЕННЫЕ] =[
        "-ца".to_string(),
        "-сы".to_string(),
        "-er".to_string(),
        "-мы".to_string(),
    ...
    //
    let исключения:[Ячейка_замены_с_исключением;lib::СЛОВАРЬ_ПЕРЕНОСОВ_ИСКЛЮЧЕНИЯ]= [
    ...
    return Словарь_Переносов {
       однобуквенные:привести_ряд_сло_словаря_переносов_в_стопку(&однобуквенные_ряд),
        многобуквенные:привести_ряд_сло_словаря_переносов_в_стопку(&многобуквенные_ряд),
        трехбуквенные:привести_ряд_сло_словаря_переносов_в_стопку(&трехбуквенные_ряд),
        двубуквенные:привести_ряд_сло_словаря_переносов_в_стопку(&двубуквенные_ряд),
        целиковые:привести_ряд_сло_словаря_переносов_в_стопку(&целиковые_ряд),
        исключения:исключения,
    }
}
fn привести_ряд_сло_словаря_переносов_в_стопку(ряд:&[String])-> Vec<lib::Ячейка_замены>{
let mut ряд_итоговый:Vec<lib::Ячейка_замены>=
    ряд.into_iter().map(|слово|
        {
        //
    ...
    //
            Ячейка_замены{
            re_образец:Regex::new(&format!(r##"(?i)\b{{end}}{}\b{{end}}"##,слово)).unwrap(),
            замена:замена,
...
fn поиск_повторов_re_словаря_замен (словарь_замен:&lib::Словарь_Переносов ) {
