...
//если это картинка
use crate::lib::{self, Быстрый_Словарь, Полный_Словарь, Словарь_разделителей, Счётчик_замен, Счётчик_разделителей, Ячейка_словаря};
...
pub fn добавить_разделители(
    ...
    содержимое.par_iter_mut().enumerate().for_each(|(указатель_строки, строка)| {
        // Перебираем все ячейки словаря последовательно (для текущей строки)
        for (указатель_ячейки, ячейка) in словарь_разделителей.ряд_1.iter().enumerate() {
            ...
            if sz_найти(строка,&ячейка.искомое_слово) {
                // Если есть совпадение с основным regex
                    // Проверяем, не попадает ли строка под исключения
                        ...
                        // Выполняем замену
                        let замененная_строка: Cow<str> = ячейка.re_образец_для_замены.replace_all(строка, "$1-$2");
                        ...
                        if замененная_строка != *строка {
                            счётчики_замен.подсчёт[указатель_ячейки].fetch_add(1, Ordering::Relaxed);
                        }
        ...
        .for_each(|(указатель, строка)| {
            ...
            if sz_найти(&строка, &знак_переноса) {
                for указатель_образца in 0..словарь_замен.исключения.len()
                {
                    ...
                    let re_образец = &словарь_замен.исключения[указатель_образца].re_образец_для_поиска;
                    let искомое_слово =
...
pub fn создать_словарь_разделителей()->Словарь_разделителей {
    ...
    use crate::dictionary_0::проверка_ряда_regex;
    let ряд_1: Словарь_разделителей = Словарь_разделителей {
        ряд_1:[
            Ячейка_замены_с_разделителями {
                искомое_слово: "пред".to_string(),
                замена: "пред-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}пред(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(пред)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "тепло".to_string(),
                замена: "тепло-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}тепло(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(тепло)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "право".to_string(),
                замена: "право-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}право(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(право)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "сухо".to_string(),
                замена: "сухо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}сухо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(сухо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "крово".to_string(),
                замена: "крово-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}крово(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(крово)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "кратко".to_string(),
                замена: "кратко-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}кратко(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(кратко)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                ...
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(ино)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "взрыво".to_string(),
                замена: "взрыво-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}взрыво(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(взрыво)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "мало".to_string(),
                замена: "мало-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}мало(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(мало)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                ...
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(без)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                ...
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(бес)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "долго".to_string(),
                замена: "долго-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}долго(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(долго)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                ...
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(умо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "сверх".to_string(),
                замена: "сверх-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}сверх(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(сверх)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "воздухо".to_string(),
                замена: "воздухо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}воздухо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(воздухо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "соот".to_string(),
                замена: "соот-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}соот(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(соот)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                ...
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(бое)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "взаимо".to_string(),
                замена: "взаимо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}взаимо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены:Regex::new(r"\b{start}(взаимо)([\w]{4,})").unwrap(),
            },
        Ячейка_замены_с_разделителями {
        искомое_слово: "само".to_string(),
        замена: "само-".to_string(),
        re_образец_для_поиска: Regex::new(r"\b{start}само(\w){4,}").unwrap(),
            re_исключение:vec![],
            re_образец_для_замены:Regex::new(r"\b{start}(само)([\w]{4,})").unwrap(),
    },
            Ячейка_замены_с_разделителями {
                искомое_слово: "хитро".to_string(),
                замена: "хитро-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}хитро(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(хитро)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "противо".to_string(),
                замена: "противо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}противо(\w){3,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(противо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "пожаро".to_string(),
                замена: "пожаро-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}пожаро(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(пожаро)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "самолето".to_string(),
                замена: "самолето-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}самолето(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(самолето)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "самолёто".to_string(),
                замена: "самолёто-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}самолёто(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(самолёто)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "тазо".to_string(),
                замена: "тазо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}тазо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(тазо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "прапра".to_string(),
                замена: "прапра-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}прапра(\w){3,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(прапра)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "миро".to_string(),
                замена: "миров".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}миро(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(миро)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "пере".to_string(),
                замена: "пере-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}пере(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(пере)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "народо".to_string(),
                замена: "народо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}народо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(народо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "верхо".to_string(),
                замена: "верхо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}верхо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(верхо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "старо".to_string(),
                замена: "старо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}старо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(старо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "много".to_string(),
                замена: "много-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}много(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(много)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "выше".to_string(),
                замена: "выше-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}выше(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(выше)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "разно".to_string(),
                замена: "разно-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}разно(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(разно)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "высоко".to_string(),
                замена: "высоко-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}высоко(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(высоко)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "родо".to_string(),
                замена: "родо-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}родо(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(родо)([\w]{4,})").unwrap(),
            },
            Ячейка_замены_с_разделителями {
                искомое_слово: "узко".to_string(),
                замена: "узко-".to_string(),
                re_образец_для_поиска: Regex::new(r"\b{start}узко(\w){4,}").unwrap(),
                re_исключение:vec![],
                re_образец_для_замены: Regex::new(r"\b{start}(узко)([\w]{4,})").unwrap(),
            },
    ...
    //проверка образцов
    проверка_ряда_regex_разделителей(&однобуквенные, "проверка разделителей");
...
use crate::xlsx::import_xlsx::{найти_особые_знаки, обратно_убрать_особые_знаки,};
use crate::import::functions::преобразовать_слово_с_чертой_в_конце;

pub fn создать_второй_словарь_разделителей(mut словарь_изначальный:Словарь_разделителей) ->Словарь_разделителей {
    словарь_изначальный.ряд_1.par_iter_mut()
        .enumerate()
        .for_each(|(указатель, ячейка)| {
            ...
            let новый_образец:String = format!(r#"\b{{start}}({})([\w]{{4,}})"#,ячейка.искомое_слово);
            ячейка.re_образец_для_замены=Regex::new(&новый_образец).unwrap();
            ячейка.замена =обратно_убрать_особые_знаки(преобразовать_слово_с_чертой_в_конце(временный_составной_ряд.0.to_case(Case::Sentence)));
        });
...
pub fn создать_второй_словарь_переносов(
    ...
    словарь_переносов
        ...
        .for_each(|(указатель, ячейка)| {
            ...
            ячейка.re_образец_для_поиска = Regex::new(&новый_образец).unwrap();
            ячейка.замена = ячейка.замена.replace('-', "—");
            ячейка.re_исключение =
                ячейка.re_исключение.iter().map(|строка|
                    Regex::new(&строка.replace("-", "—")).unwrap())
                    .collect();
        });
    словарь_переносов
        .двубуквенные
...
pub fn создать_третий_словарь_переносов(
    ...
    словарь_переносов
        ...
        .for_each(|(указатель, ячейка)| {
            ячейка.искомое_слово = ячейка.искомое_слово.replace('-', &на_что_заменять);
            let новый_образец = ячейка.re_образец_для_поиска.as_str().replace("-", &на_что_заменять);
            ячейка.re_образец_для_поиска = Regex::new(&новый_образец).unwrap();
            ячейка.замена = ячейка.замена.replace('-', &на_что_заменять);
            ячейка.re_исключение =
                ячейка.re_исключение.iter().map(|строка|
                    Regex::new(&строка.replace("-", &на_что_заменять)).unwrap())
                    .collect();
        });
    словарь_переносов
        .двубуквенные
    ...
    let куча: HashSet<String> = (0..ряд.len())
        ...
        .flat_map(|i| {
            ...
            if !sz_найти(&ряд[i].to_string(), r"\b{start}") {
                куча_2.insert(format!(r##"Regex нет знака начала слова \b{{start}}: {}"##, ряд[i]));
            }
...
pub fn добавить_слова_с_окончаниями () {
    ...
    impl Default for окончания {
        fn default() -> Self {
            Self {
                щ: ["щ", "ща", "щая", "щую", "ще", "щем", "щему", "щего", "щее", "щей",
                    "щесть", "щестью", "щести", "щестью", "щесстям","щесстями","щесстях",].map(String::from),
            }
        }
    }
}
