...
pub fn md_получить_указатели_на_пропуки(
    ...
    for указатель in 0..содержимое.len() {
        ...
        if md_проверка_содержимого_на_условия(&содержимое[указатель])
        {
            пропуски.insert(указатель);
        }
    }
...
pub fn проверка_исключений_в_стоге_сена(
    ...
    for указатель in 0..исключения.len() {
        if sz_найти(стог_сена, &исключения[указатель]) {
            //теперь уже RE выражения
            if re_исключения[указатель].is_match(стог_сена) {
                return true;
            }
        }
    }
...
fn проверка_образцов_для_кучи(
    ...
    for указатель in 0..исключения.len() {
        ...
        if !sz_найти(
            ...
            panic!(
                "md: Re образец:{} не соответствует обычному образцу: {}, порядковый указатель: {указатель}",
                исключения_re[указатель], исключения[указатель]
            )
        }
        //сопоставление 2-х рядов
        for указатель_2 in указатель + 1..исключения.len() {
            if исключения[указатель].as_str() == исключения[указатель_2]
            {
                panic!()
            }
        }
    ...
    //перебор RE
    for указатель in 0..исключения_re.len() {
        for указатель_2 in указатель + 1..исключения_re.len() {
            if исключения_re[указатель].as_str() == исключения_re[указатель_2].as_str()
            {
                panic!()
            }
        }
...
pub fn есть_ли_кириллица(стог_сена: &String) -> bool {
    ...
    for образец in малые_буквы.iter() {
        if sz_найти(&стог_сена, &образец.to_string()) {
            return true;
        }
    }
    for образец in большие_буквы.iter() {
        if sz_найти(&стог_сена, &образец.to_string()) {
            return true;
        }
    }
