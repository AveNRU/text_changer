...
pub fn определить_кодировку(
    ...
    if расширение_подробно.архив_книга() || без_кодировки(&имя)
    //|| !sz_найти(&имя, ".")
    {
        /*println!("Имя |{}| Путь |{}| - UTF8", имя, путь);

        println!(
            "расширение_подробно.архив_книга()  |{}|",
            расширение_подробно.архив_книга()
        );
        println!("!sz_найти(&имя)  |{}|", !sz_найти(&имя, "."));
        println!("без_кодировки(&имя)  |{}|", без_кодировки(&имя));*/
    ...
    for указатель in 0..конец {
        if let Some(строка) = ряд_строк.get(указатель) {
            ...
            {
                // println!("Имя |{}| Путь |{}| - 1251", имя, путь);
    ...
    */
    ...
    // println!("Имя |{}| Путь |{}| - UTF8 итог", имя, путь);
    return Text_Changer::Кодировка::Utf8;
}
...
pub fn htm_utf8_без_переносов_строк(
    ...
    let mut итог: Vec<String> = удалить_лишние_пробелы(&итог);
    if калибри {
        // return удалить_переносы_калибри(&итог);
        итог = удалить_переносы_калибри(&итог); //07.09.2026 проверка
    }
    ...
    let итог: Vec<String> =
        удалить_рекламу_после_разбиения_строк(итог);
    //удаляем висящие переходы
    if расширение_подробно.архив_книга() {
        //
        //println!("Зашло менять |{}|", имя_файла);
        let итог: Vec<String> = удалить_переходы_в_epub(итог).unwrap();
        return удаление_script_мусора_после_разбиения_строк(
            итог,
            &имя_файла,
        );
    }
    //иначе без удаления и склеивания для Epub
    else {
        return удаление_script_мусора_после_разбиения_строк(
            итог,
            &имя_файла,
...
fn удалить_рекламу_после_разбиения_строк(
    ...
    for строка_ряда in итог.iter() {
        //if итог[i].is_empty() {continue} else {итог2.push(итог[i].clone())}
        if если_пустая_строка_с_отделителями(&строка_ряда) {
            continue;
        } else {
            итог2.push(строка_ряда.clone())
        }
    }
    //удалить скрипты
...
use Text_Changer::Ячейка_замены_примечания;
pub fn удалить_разделы_js_по_ключевым_словам_с_повторами(
    mut ряд: Vec<String>,
    заготовленный_ряд: &Ячейка_замены_примечания,
    _имя_файла: &String,
) -> Vec<String> {
    //перебор всего ряда
    'главный_указатель: for строка in ряд.iter_mut() {
        //вложениян начало
        if sz_найти(строка, &заготовленный_ряд.начало) {
            //перебор исключений
            '_указатель_круговорота: for исключение in
                заготовленный_ряд.исключение.iter()
            {
                //если есть исключение - пропустить
                if sz_найти(строка, &исключение) {
                    continue 'главный_указатель;
                }
            }
            //если не нашло исключение - то замена
            let строка2 = строка.replace(заготовленный_ряд.начало, заготовленный_ряд.замена);
            *строка = строка2;
        }
        // вложения конец
    }
...
fn удаление_script_мусора_после_разбиения_строк(
    ...
    use Text_Changer::{
        Ячейка_замены_переносов, Ячейка_замены_примечания
    };
    //
    ...
    //
    static ЗАГОТОВЛЕННЫЙ_РЯД_JS: LazyLock<[Ячейка_замены_примечания; 9]> = LazyLock::new(|| {
        [
            Ячейка_замены_примечания {
                ...
                исключение: vec!["windows-1251", "windows-1252", "utf8"],
            },
            Ячейка_замены_примечания {
                начало: r##"<script src="##,
                // начало_re: Regex::new(r##"([^'\"])<script"##).unwrap(),
            ...
            //
            Ячейка_замены_примечания {
                начало: r##"<script type="text/javascript">"##,
                // начало_re: Regex::new(r##"([^'\"])<script"##).unwrap(),
                //вложение: &РАЗДЕЛИТЕЛЬ_SCRIPT,
            ...
            //
            Ячейка_замены_примечания {
                начало: r##"<script async="""##,
                // начало_re: Regex::new(r##"([^'\"])<script"##).unwrap(),
                //вложение: &РАЗДЕЛИТЕЛЬ_SCRIPT,
                замена: r##"<script /* async="""##,
                исключение: vec!["windows-1251", "windows-1252", "utf8"],
                //примечания: Примечания::js,
            },
            Ячейка_замены_примечания {
                начало: r##"<script type="text/javascript"  async="""##,
                // начало_re: Regex::new(r##"([^'\"])<script"##).unwrap(),
                //вложение: &РАЗДЕЛИТЕЛЬ_SCRIPT,
                замена: r##"<script /* type="text/javascript"   async="""##,
                исключение: vec!["windows-1251", "windows-1252", "utf8"],
                //примечания: Примечания::js,
            },
        ]
...
use Text_Changer::{
    Ячейка_замены_Epub, Ячейка_замены_переносов_Epub
};
pub fn удалить_переходы_в_epub(
    ряд_начальный: Vec<String>,
) -> Result<Vec<String>, String> {
    let mut итог: Vec<String> = ряд_начальный.clone();
    static КОНЕЦ_СТРОКИ_С_ЧЕРТЫ: LazyLock<Regex> = LazyLock::new(|| Regex::new(r##"-$"##).unwrap());
    ...
    'главный_указатель: for указатель in 0..итог.len() {
        //
        for образец_начальный in ИСКЛЮЧЕНИЯ.образцы_начала.iter()
        {
            //находит ли конец строки
            if sz_найти(&итог[указатель], образец_начальный.искомое_слово.as_str())
            //если нашлось
            {
                //
                let следующий_указатель: usize = указатель + 1;
                //проверка безопасного доступа
                match итог.get(указатель + 1) {
                    Some(_) => (),
                    None => {
                        //итог.push(ряд[указатель].clone());
                        continue 'главный_указатель;
                    }
                }
                //
                //let следующая_строка: &mut String = &ряд[следующий_указатель];
                //
                for образец_конца in ИСКЛЮЧЕНИЯ.образцы_конца.iter()
                //
                {
                    //ищем маленьки буквы в следующей стркое
                    if sz_найти(
                        &итог[следующий_указатель],
                        образец_конца.искомое_слово.as_str(),
                    ) {
                        //ищем маленьки буквы в следующей стркое через re
                        if образец_конца
                            .re_образец_поиска
                            .is_match(&итог[следующий_указатель])
                        //если нашлось совпадение
                        {
                            //удаляем пробелы все через RE
                            /*let начальная_строка: String = итог[указатель]
                            .replace(образец_начальный.искомое_слово.as_str(), "");*/
                            let начальная_строка_правленная_regex: String =
                                образец_начальный
                                    .re_образец_замены
                                    .replace_all(&итог[указатель], "")
                                    .to_string();
                            /*let вторая_строка: String = итог[следующий_указатель]
                            .replace(образец_конца.искомое_слово.as_str(), "");*/
                            let вторая_строка_правленная_regex: String = образец_конца
                                .re_образец_замены
                                .replace_all(&итог[следующий_указатель], "")
                                .to_string();
                            //если есть разделитель в виде черты
                            if КОНЕЦ_СТРОКИ_С_ЧЕРТЫ.is_match(&начальная_строка_правленная_regex)
                                || НАЧАЛО_СТРОКИ_С_ЧЕРТЫ.is_match(&вторая_строка_правленная_regex)
                            {
                                итог[указатель] = format!(
                                    "{}{}",
                                    начальная_строка_правленная_regex,
                                    вторая_строка_правленная_regex
                                );
                            }
                            //если нет черты - то вставляем 1 пробел
                            else {
                                //
                                итог[указатель] = format!(
                                    "{} {}",
                            ...
                            //println!("замена удалась");
                            итог[следующий_указатель] = format!("<!--*здесь была строка*-->",);
                            //ряд[следующий_указатель] = вторая_строка;
                            //итог.push(начальная_строка);
                            // итог.push(вторая_строка);
                            //следующая строка
                            continue 'главный_указатель;
                        }
                    }
...
pub fn добавить_переносы_строк_html(
    ...
    //
    const ОБРАЗЦЫ_ДО: [&str; 1] = [r#"<img"#];
    // r#"content="text/html; charset=UTF-8">"#,
    const ОБРАЗЦЫ_ПОСЛЕ: [&str; 115] = [
        r#"<form>"#,
        r#" alt="#,
        /*r#" src="#,
        r#" data-img="#,
    ...
    if !СЧЁТЧИК_ПРОВЕРКИ.swap(true, Ordering::SeqCst) {
        есть_ли_повторно_строка_в_ряде(
            &ОБРАЗЦЫ_ПОСЛЕ.as_parallel_slice(),
    ...
    for образец in ОБРАЗЦЫ_ПОСЛЕ.iter() {
        новый_ряд_строк = разбить_строки_через_образец_после(
            новый_ряд_строк,
            &образец,
            true,
        );
    ...
    //
    //прогон через все образцы - полностью прогон, так как всё остальное
    for образец in ОБРАЗЦЫ_ДО.iter() {
        новый_ряд_строк = разбить_строки_через_образец_до(
            новый_ряд_строк,
            &образец,
            true,
        );
...
pub fn разбить_строки_через_образец_после(
...
pub fn разбить_строки_через_образец_до(
    исходный_ряд_строк: Vec<String>,
    образец: &str,
    полностью: bool, //до какой строки перебирать
) -> Vec<String> {
    ...
    for (_, строка_содержимое) in исходный_ряд_строк
        .iter()
        .enumerate()
        .filter(|(указатель_внутри, _)| *указатель_внутри <= конец)
    {
        if sz_найти(&строка_содержимое, &образец) {
            let (первая_строка, вторая_строка) = строка_содержимое
                .split_once(образец)
                .unwrap_or((строка_содержимое, ""));
            //   println!("нашло div");
            новый_ряд_строк.push(первая_строка.to_string());
            новый_ряд_строк.push(format!("{}{}", образец, вторая_строка));
            continue;
        } else {
            новый_ряд_строк.push(строка_содержимое.clone())
        }
    }
    return новый_ряд_строк;
}
...
fn удалить_переносы_из_вектора(
    ...
    for исключение in РЯД.iter() {
        строки.par_iter_mut().for_each(|строка| {
            let замененная_строка = исключение.replace_all(&строка, "");
