...
pub fn есть_ли_примечания_html(стог_сена:&String) -> bool{
    ...
    lazy_static!{
           static ref образцы_re_составные:[образец_сложный;1]=[
            образец_сложный{
    ...
    for i in 0..образцы_re_составные.len() {
        if образцы_re_составные[i].начало.is_match(&стог_сена) && образцы_re_составные[i].конец.is_match(&стог_сена)
...
fn удаление_script_мусора_после_разбиения_строк(
    ...
    use crate::lib::{
        Ячейка_замены_объявления, Ячейка_замены_переносов
    };
    lazy_static! {
