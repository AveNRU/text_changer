...
pub fn заменить_слова_в_книге(
    ...
    println!("замена: счётчик_вездесущее {}",полный_словарь.счётчик_вездесущее.len());
    println!("замена: счётчик_простое {}",полный_словарь.счётчик_простое.len());
    println!("замена: счётчик_составное {}",полный_словарь.счётчик_составное.len());
    println!("замена: счётчик_составное_важное {}",полный_словарь.счётчик_составное_важное.len());
    //перебор
            ...
            книга_взятая.вложения.par_iter_mut().enumerate()
                ...
                .for_each(|(указатель, вложения)| {
                    ...
                    if счётчик_количества_вложенных_файлов>1 {
                        ...
                        //pb.set_message(format!("Обработка элемента {}", i));
                        //pb.set_message(сообщение_текущее_вложение.clone());
                        //pb.inc(1);
