...
pub fn заменить_слова_в_книге(
    ...
    for указатель in 0..ряд_исходных_книг.len() {
        // for i in 0..ряд_исходных_книг.len() {
        let расширение: String = ряд_исходных_книг[указатель].расширение.clone();
        let mut указатель_захода: Mutex<usize> = Mutex::new(1);
        if ряд_исходных_книг[указатель].расширение.contains("doc") {} else {
            //временный вектор для хранения слов книг
            let mut временный_ряд_книг: Vec<lib::Вложения> = ряд_исходных_книг[указатель].вложения.clone();
            ...
            временный_ряд_книг.par_iter_mut().enumerate().for_each(|(указатель, значение)| {
                ...
                println!(
                    ...
                    style(&ряд_исходных_книг[указатель].путь).cyan(),
                    LOOKING_GLASS
                );
                ...
                let куча_пропусков: HashSet<usize> = utils::hash::проверка_содержимого_в_зависимости_от_расширения_книги(
                    &значение.содержимое,
                    &ряд_исходных_книг[указатель].расширение);
                ...
                crate::output::dir::создать_папку_книги(
                    &ряд_исходных_книг[указатель].название_книги,
                    &ряд_исходных_книг[указатель].расширение,
                );
                let mut путь_вывода_пропусков = format!(
                    "{}{}/пропуски.txt",
                    &пути_общие.вывод_пропуски, &ряд_исходных_книг[указатель].название_книги
                );
                if sz_найти(&ряд_исходных_книг[указатель].название_книги, "index")
                {
            ...
            // if временный_ряд_книг[0].содержимое==ряд_исходных_книг[указатель].вложения[0].содержимое {println!("совпадают книги")  }
            ...
            /* let временная_книга = lib::Книги {
                ...
                путь: ряд_исходных_книг[указатель].путь.clone(),
                название_книги: ряд_исходных_книг[указатель].название_книги.clone(),
                расширение: ряд_исходных_книг[указатель].расширение.clone(),
            };*/
            //вложение в общую стопку
            //стопка_книг_измененная.push(временная_книга);
            for k2 in 0..временный_ряд_книг.len() {
                if временный_ряд_книг[k2].содержимое == ряд_исходных_книг[указатель].вложения[k2].содержимое {
                    println!("Ошибка - книги равны")
                }
            }
        }
