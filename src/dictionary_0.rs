//use std::default;
use crate::lib::{self, Полный_Словарь, Словарь, Сообщения_для_книги, Счётчики_Словаря};
...
pub fn заменить_слова_в_книге(
    ...
    let лупа = format!("🔍");
    //создание счётчиков
    let mut счётчики_словаря:Arc<lib::Счётчики_Словаря>=создать_счётчики_словаря(&полный_словарь);
        ...
        //остальные расширения
            ...
            книга_взятая.вложения.par_iter_mut().enumerate()
                ...
                .for_each(|(указатель, вложения)| {
                    ...
                    let mut счётчики_замен_вложенные: [Arc<Счётчик_замен>;2] =
                        [счётчики_замен[0].clone(),
                            счётчики_замен[1].clone()];
                ...
                вывод_содержимого_строки_в_txt(
                    ...
                    //for указатель_переносов in 0..словарь_переносов.len() {
                       // println!("заход: {указатель_переносов}: {:?}",счётчики_замен_вложенные);
                        убрать_переносы(
                            &словарь_переносов[0],
                            ...
                            &mut счётчики_замен_вложенные[0],
     ...
     */
    write::вывод_всех_словарей_в_xls(&полный_словарь, &куча_словарь,&счётчики_словаря).unwrap();
    for указатель_переносов in 0..словарь_переносов.len() {
