...
use crate::lib::{
    ...
    Словарь_разделителей, Сообщения_для_книги, Счётчик_разделителей, Счётчики_Словаря,
    Ячейка_словаря,
};
...
use crate::xlsx::import_xlsx::{найти_особые_знаки, обратно_убрать_особые_знаки, поиск_уже_добавленных_слов};
...
pub fn заменить_слова_в_книге_и_их_вывод(
    ...
    //
    let mut новые_сообщения= crate::write::сохранить_книги(&книги).unwrap();
    //
    ...
    */
    write::вывод_всех_счётчиков_разделителей_в_xls(
        ...
        .unwrap();
    //
    let сообщения: lib::Сообщения = Arc::try_unwrap(временные_сообщения)
        .unwrap()
        .into_inner()
        .unwrap();
    ...
    */
    ...
    /*rayon::spawn(move || {
            crate::dictionary_0::проверка_на_пересечения_в_словаре(полный_словарь)
    });*/
    crate::dictionary_0::проверка_на_пересечения_в_словаре(полный_словарь);
...
pub fn вложить_слова_искомые_в_кучу_словарь(
    полный_словарь_искомые: &mut [Куча_Словарь_Искомые; 3],
...
pub fn условие_добавления_ошибки_в_кучу_при_повторе_слова_в_словаре(
   ...
   return match имя_страницы {
        Имена_страниц::простое=> какое_правописание(указатель_правописания),
        Имена_страниц::составное=>какое_правописание(указатель_правописания),
        Имена_страниц::составное_важное=>какое_правописание(указатель_правописания),
       Имена_страниц::огласовки=>какое_правописание(указатель_правописания),
        Имена_страниц::вездесущее=>return какое_правописание(указатель_правописания),
    ...
    fn какое_правописание(    указатель_правописания: &Правописание_слова,)->bool {
...
pub fn проверить_совпадений_слова_замен_и_искомых_слов_в_куче_во_всех_словарях(
    ...
    /* println!("Искомая куча - число");
    for (ярус,словарь) in куча_словарь_искомые.iter().enumerate() {
        println!("Ярус: {}", ярус);
        println!("Простых: всего - {}", словарь.простое.len());
        println!("Огласовки: всего - {}", словарь.огласовки.len());
        println!("Составных: всего - {}", словарь.составное.len());
        println!("Составных сложных: всего - {}", словарь.составное_важное.len());
        println!("Неизменных: всего - {}", словарь.неизменное.len());
        println!("Неизменных длинных: всего - {}", словарь.неизменное_длинное.len());
        println!("Неизменных коротких: всего - {}", словарь.неизменное_короткое.len());
        println!("Вездесущих: всего - {}", словарь.вездесущее.len());
    }
    println!("Куча замен - число");
    for (ярус,словарь) in куча_словарь_искомые.iter().enumerate() {
        println!("Ярус: {}", ярус);
        println!("Простых: всего - {}", словарь.простое.len());
        println!("Огласовки: всего - {}", словарь.огласовки.len());
    ...
    let куча: rapidhash::fast::RapidHashSet<String>=
        полный_словарь.простое.par_iter().enumerate().flat_map(|(указатель_искомого_яруса,раздел_искомого)|{
                указатели_словаря.par_iter().enumerate()
                ...
                ячейки_словаря.par_iter().filter_map(|ячейка_словаря| {
    ...
    /*
    let куча: rapidhash::fast::RapidHashSet<String>=
        куча_словарь_искомые.par_iter().enumerate().flat_map(|(указатель_искомого_яруса,раздел_искомого)|{
            раздел_искомого.простое.iter().flat_map(|слово|{

            куча_словарь_замены.par_iter().enumerate().flat_map(|(указатель_яруса_замены, раздел_замены)| {
            let указатели_словаря: [&RapidHashSet<String>; 6] = [
                &раздел_замены.составное,
                &раздел_замены.составное_важное,
                &раздел_замены.вездесущее,
                &раздел_замены.неизменное,
                &раздел_замены.неизменное_короткое,
                &раздел_замены.неизменное_длинное,
            ];

            let указатель_поиска: &RapidHashSet<String> = &раздел_искомого.простое;
            //
            указатели_словаря.iter().enumerate().flat_map(|(указатель_раздела_кучи,куча_сама)| {
                куча_сама.iter().filter_map(|сама_строка| {
                    //
                     if sz_найти(сама_строка,слово)
                    {
                        println!("{}",format!("Словарь : |{}| содержит искомое простое слово: |{}| в заменах | Пересечение всего словаря", раздел_словаря.to_string(), слово));
                        Some(format!("Словарь : |{}| содержит искомое простое слово: |{}| в заменах | Пересечение всего словаря", раздел_словаря.to_string(), слово))
                    } else {None}
    ...
    return куча;
    fn разрешить_ли_проход(искомое_слово:&String, указатели_словаря:usize) ->bool {
