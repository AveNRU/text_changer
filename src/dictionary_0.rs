...
pub fn заменить_слова_в_книге(
    ...
    *сообщения = Arc::try_unwrap(временные_сообщения)
    ...
    println!(
        "{}{}",
        style(format!("\tВыделено памяти на этапе [3/4]: Замена слов по словарю:")).bold(),
    ...
    let полный_словарь = ряд_словарей
        ...
        .fold_with(
            ...
            |mut накопитель, ячейка| {
                ...
                накопитель.неизменное_короткое.extend(ячейка.неизменное_короткое);
                накопитель.неизменное_длинное.extend(ячейка.неизменное_длинное);
                накопитель
            },
        )
        .reduce(
...
fn получить_кучи_из_словарей(
    ...
    //неизменное короткое
    let неизменное_короткое: HashMap<String, HashSet<usize>> = создать_быстрый_словарь(
        &полный_словарь
            .неизменное_короткое
            .par_iter()
            .map(|ячейка| ячейка.искомое_слово.to_string())
    ...
    //неизменное длинное
    let неизменное_длинное: HashMap<String, HashSet<usize>> = создать_быстрый_словарь(
        &полный_словарь
            .неизменное_длинное
            .par_iter()
            .map(|ячейка| ячейка.искомое_слово.to_string())
