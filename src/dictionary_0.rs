...
pub fn заменить_слова_в_книге(
    ...
    use crate::utils::stringzilla::{sz_упорядочить_словарь,sz_найти};
    //шкала
    let mut временные_сообщения:Mutex<lib::Сообщения> =Mutex::new(сообщения.clone());
    //
    use crate::utils::regex::{
        замена_слов_через_regex, замена_слов_через_кучу
    };
    ...
    //упорядочить словарь
    let mut полный_словарь=sz_упорядочить_словарь(полный_словарь);
    //быстрый словарь
      let куча_словарь:lib::Куча_Словарь=получить_кучи_из_словарей(&полный_словарь);
    //начало замены слов
    //перебор книг
    println!("составное важное всего: {}",полный_словарь.составное_важное.len());
    static LOOKING_GLASS: Emoji<'_, '_> = Emoji("🔍  ", "");
    let счётчик_составное_важное:Mutex<Vec<usize>>=Mutex::new(полный_словарь.счётчик_составное_важное.clone());
    ...
    let счётчик_вездесущее:Mutex<Vec<usize>>=Mutex::new(полный_словарь.счётчик_вездесущее.clone());
    println!("счётчик_составное_важное:{:?}",счётчик_составное_важное.lock().unwrap().len());
    //перебор
    ряд_исходных_книг.par_iter().enumerate().for_each(|(указатель,книга_взятая)|{
        ...
        if книга_взятая.расширение.contains("doc") {
           ...
           // for j in 0..временный_ряд_книг.len() {
                ...
                for указатель in куча_пропусков.iter() {
                    пропущенные_строки
                        .push(значение.содержимое[*указатель].clone());
                }
                пропущенные_строки =
            ...
            // if временный_ряд_книг[0].содержимое==книга_взятая.вложения[0].содержимое {println!("совпадают книги")  }
            ...
            /* let временная_книга = lib::Книги {
                ...
                путь: книга_взятая.путь.clone(),
                название_книги: книга_взятая.название_книги.clone(),
...
pub fn создать_быстрый_словарь(
    ...
    let mut ряд_временный: HashSet<String> = HashSet::new();
    //
    ...
    let путь_простой:String=format!("{}{}.txt",
                            &пути_общие.вывод_кучи_словаря,
        вид_слов,
    );
    let путь_ключи:String=format!("{}{}.txt",
...
fn получить_кучи_из_словарей (полный_словарь: &ПолныйСловарь)
    ->lib::Куча_Словарь {
    ...
    rayon::scope(|s| {
        ...
        s.spawn(|_| {
            let словарь = создать_быстрый_словарь(&полный_словарь.простое, "простые");
            результаты_ref.lock().unwrap().0 = Some(словарь);
        });

        s.spawn(|_| {
            let словарь = создать_быстрый_словарь(&полный_словарь.составное, "составные");
            результаты_ref.lock().unwrap().1 = Some(словарь);
        });

        s.spawn(|_| {
            let словарь = создать_быстрый_словарь(&полный_словарь.составное_важное, "составные_важные");
            результаты_ref.lock().unwrap().2 = Some(словарь);
        });

        s.spawn(|_| {
            let словарь = создать_быстрый_словарь(&полный_словарь.вездесущее, "вездесущие");
