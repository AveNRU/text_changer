...
pub fn заменить_слова_в_книге(
    ...
    //перебор
    let количество_книг =ряд_исходных_книг.len();
    ряд_исходных_книг.par_iter_mut().enumerate().for_each(|(указатель,книга_взятая)|{
   // for i in 0..ряд_исходных_книг.len() {
        ...
        if книга_взятая.расширение.contains("doc") {
            ...
            книга_взятая.вложения.par_iter_mut().enumerate().for_each(|(указатель, вложения)| {
           // for j in 0..временный_ряд_книг.len() {
                //имя книги
                let текущий_шаг_всех_книг = format!("[{}/{}]", указатель + 1, количество_книг);
                println!(
                    "{}: {} {}",
            ...
            проверка_есть_ли_изменения(&вложения_изначальные,&книга_взятая.вложения);
    ...
    *сообщения = временные_сообщения.into_inner().unwrap();
    ...
    fn проверка_есть_ли_изменения(содержимое_изначальное:&Vec<lib::Вложения>,содержимое_изменённое:&Vec<lib::Вложения>,
    имя_книги:&String) {
        for k2 in 0..содержимое_изначальное.len() {
            if содержимое_изначальное[k2].содержимое == содержимое_изменённое[k2].содержимое { 
                println!("Книга: {} замены не были произведены",имя_книги) }
            else {
                println!("Книга: {} были совершены замены",имя_книги)
            }
        }
    }
}
