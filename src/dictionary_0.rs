...
pub fn заменить_слова_в_книге(
            ...
            книга_взятая.вложения.par_iter_mut().enumerate()
                ...
                вывод_содержимого_строки_в_txt(
                    ...
                    замена_слов_через_кучу(
                        ...
                        // &mut счётчик_неизменное_лок,
                        //"[0/4] Составные важные слова",
                        &format!("{} | [2/9] Неизменные длинные слова",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                    ...
                    замена_слов_через_кучу(
                        ...
                        // &mut счётчик_неизменное_лок,
                        //"[0/4] Составные важные слова",
                        &format!("{} | [1/9] Неизменные короткие слова",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                ...
                замена_слов_через_кучу(
                    ...
                    &format!("{} | [4/9] Составные важные слова",сообщение_текущее_вложение),
                    &книга_взятая.расширение,
                    &куча_пропусков,
                ...
                замена_слов_через_кучу(
                    ...
                    &format!("{} | [5/9] Вездесущие слова",сообщение_текущее_вложение),
                    &книга_взятая.расширение,
                    &куча_пропусков,
    ...
    .unwrap();
    полный_словарь.огласовки.clear();
    полный_словарь.простое.clear();
    полный_словарь.вездесущее.clear();
    полный_словарь.неизменное.clear();
    полный_словарь.неизменное_короткое.clear();
    полный_словарь.неизменное_длинное.clear();
    полный_словарь.составное.clear();
    полный_словарь.составное_важное.clear();
...
pub fn создать_быстрый_словарь(
    ...
    let условие:bool=if sz_найти(&вид_слов.to_string(),"оглас") {true} else {false};
...
pub fn utf8_char_len(s: &str) -> usize {
...
pub fn выделить_кучу_из_ряда_для_словаря(
    ...
    for (указатель, исходное_слово) in ряд_слов.iter().enumerate() {
        ...
        let слово = if utf8_char_len(&исходное_слово) > 3 {
            выделить_окончание_из_слова(исходное_слово, &mut счётчик_входа)
            //
            /*if условие {
                if sz_найти(&исходное_слово,"ее")||sz_найти(&исходное_слово,"Ее") {
                println!("исходное_слово: {}, его длина {}",исходное_слово,utf8_char_len(&исходное_слово));
                println!("готовое слово: {}, его длина {}",готовое_слово,utf8_char_len(&готовое_слово));}
            }*/
            }
        //если 3 буквы и меньше
        else {
            //println!("меньше 3: {}",исходное_слово);
