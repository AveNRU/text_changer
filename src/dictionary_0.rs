...
pub fn заменить_слова_в_книге(
    ...
    let mut счётчик_вездесущее: Vec<AtomicUsize> = (0..полный_словарь.счётчик_вездесущее.len())
        ...
        .map(|_| AtomicUsize::new(0))
        .collect();
    let mut счётчик_неизменное: Vec<AtomicUsize> = (0..полный_словарь.счётчик_неизменное.len())
        .into_par_iter()
        .map(|_| AtomicUsize::new(0))
        .collect();
    ...
    *сообщения = Arc::try_unwrap(временные_сообщения).unwrap().into_inner().unwrap();
    ...
    fn проверка_есть_ли_изменения(
        ...
        //let шаг_внутренний = AtomicU64::new(0);
        //создаём ряд пустой
        let mut сообщения: Arc<Mutex<Vec<String>>> = Arc::new(Mutex::new(vec![
                "".to_string();
        ...
        let mut сообщения: Vec<String> = Arc::try_unwrap(сообщения).unwrap().into_inner().unwrap();
...
pub fn добавить_все_слова_в_словарь(
    ...
    //перебор словаря
    let полный_словарь = Arc::new(Mutex::new(lib::Полный_Словарь::default()));
    ряд_словарей.into_par_iter().for_each(|ячейка| {
        if let Ok(mut словарь) = полный_словарь.lock() {
            словарь.простое.extend(ячейка.простое);
            словарь.вездесущее.extend(ячейка.вездесущее);
            словарь.составное.extend(ячейка.составное);
            словарь.составное_важное.extend(ячейка.составное_важное);
            словарь.неизменное.extend(ячейка.неизменное);
        }
    });
    ...
    return поиск_уже_добавленных_слов_в_полном_словаре(
        полный_словарь.into(), //номер страницы
    );
}
...
fn получить_кучи_из_словарей(
    ...
    let простое: Mutex<HashMap<String, HashSet<usize>>> =
        Mutex::new(HashMap::with_hasher(foldhash::fast::RandomState::default()));
    let составное: Mutex<HashMap<String, HashSet<usize>>> =
        Mutex::new(HashMap::with_hasher(foldhash::fast::RandomState::default()));
    let составное_важное: Mutex<HashMap<String, HashSet<usize>>> =
        Mutex::new(HashMap::with_hasher(foldhash::fast::RandomState::default()));
    let вездесущее: Mutex<HashMap<String, HashSet<usize>>> =
        Mutex::new(HashMap::with_hasher(foldhash::fast::RandomState::default()));
    let неизменное: Mutex<HashMap<String, HashSet<usize>>> =
        Mutex::new(HashMap::with_hasher(foldhash::fast::RandomState::default()));
    rayon::scope(|s| {
        //  let результаты_ref = &результаты;

        s.spawn(|_| {
            let словарь: HashMap<String, HashSet<usize>> =
        ...
        s.spawn(|_| {
            ...
            *составное_важное.lock().unwrap() = словарь;
        });
        ...
        s.spawn(|_| {
            ...
            *вездесущее.lock().unwrap() = словарь;
        });
        s.spawn(|_| {
            let словарь: HashMap<String, HashSet<usize>> =
                создать_быстрый_словарь(
            ...
            *неизменное.lock().unwrap() = словарь;
        });
    });
    return lib::Куча_Словарь {
        простое: простое.into_inner().unwrap(),
        составное: составное.into_inner().unwrap(),
        составное_важное: составное_важное.into_inner().unwrap(),
        вездесущее: вездесущее.into_inner().unwrap(),
        неизменное:неизменное.into_inner().unwrap(),
    };
