...
impl<'i> MyReader<'i> {
    pub fn new(input: &'i str) -> Result<Self, regex::Error> {
    ...
    pub fn read_event(&mut self) -> Result<Event<'i>, Error> {
        loop {
    ...
    /// In this example we use simple regular expression to capture entities from DTD.
    /// In real application you should use DTD parser
    pub fn capture(&mut self, doctype: BytesText<'i>) {
    ...
    pub fn resolve(&self, entity: &[u8]) -> Result<&'i [u8], EscapeError> {
    ...
    pub fn get_entity(&self, entity: &str) -> Option<&'i str> {
        self.entities
            .get(entity.as_bytes())
