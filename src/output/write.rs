...
use crate::lib::{self, Пути_Вывода, Содержимое_папок, Сообщения, Сообщения_для_книги};
use crate::utils::regex::{
    fb2_rtf_mhtml, fb3_epub, md_fs_yml, изображение_расширение_с_точкой
};
use crate::utils::zip::{Архив_в_озу, pack_zip_from_memory, zip_архив_в_память};
use encoding_rs::{
    WINDOWS_1251,
    //    DecoderResult
};
...
pub fn сохранить_книги(
    ...
    let стопки_сообщений:Mutex< Vec<crate::lib::Сообщения_для_книги>>=Mutex::new(vec![Default::default();стопки_книг.len()]);
    ...
    //for i in 0..стопки_книг.len() {
        let сообщения_запись_и_чтение:Mutex< crate::lib::Сообщения_для_книги>=Mutex::new(Default::default());
        //имя книге присваиваем для вывода
        сообщения_запись_и_чтение.lock().unwrap().имя_книги=format!("{}.{}",стопки_книг[i].название_книги,стопки_книг[i].расширение);
        //путь до вывода
...
pub fn вывод_всех_словарей_в_xls(
    ...
    for j in 0..словарь.простое.len() {
        ...
        книга.write((j + 1) as u32, 0, &словарь.простое[j].искомое_слово).unwrap();
        книга
    ...
    for j in 0..словарь.составное_важное.len() {
        ...
        составные_важные
            .write(_row_point, 0, словарь.составное_важное[j].искомое_слово.clone())
            .unwrap();
        составные_важные
            .write(_row_point, 1, словарь.составное_важное[j].re_образец.to_string())
            .unwrap();
        составные_важные
            .write(
                _row_point,
    ...
    составные_важные
        ...
        .unwrap();
    составные_важные.autofilter(0, 0, _row_point + 1, 3).unwrap();
    //путь сохранения
...
pub fn вывод_содержимого_папок_по_умолчанию(
    ...
    вывод_содержимого_в_txt(&содержимое_папок.ошибки, &путь, &mut сообщения, false).unwrap();
    //сами файлы
...
fn запись_если_есть_разница(
    ...
    if сравнение_двух_рядов_построчно(&новый_ряд_строк,&существующий_файл_ряд_строк,&путь){
...
pub fn вывод_всей_стопки_сообщений_в_txt(mut сообщения: Сообщения) -> Result<bool, Box<dyn std::error::Error>> {
   ...
   сообщения.проверка_после_замен.into_par_iter().enumerate().for_each(|(указатель,вложение)|{
       вывод_содержимого_в_txt(
           &вложение.сообщения,
           &format!("{}{}.txt",пути_вывода.вывод_книг_проверки_замен,вложение.имя_книги),
    ...
    //вывод сообщений после замен
    сообщения.запись_и_чтение.into_par_iter().enumerate().for_each(|(указатель,вложение)|{
        вывод_содержимого_в_txt(
            &вложение.сообщения,
            &format!("{}{}.txt", пути_общие.вывод_книги_запись_и_чтение, вложение.имя_книги),
            &mut сообщения_общего_выполнения.lock().unwrap(),
            true,
        )
            .unwrap();
    ...
    сообщения.общие.extend(сообщения_общего_выполнения.into_inner().unwrap());
