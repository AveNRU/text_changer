...
use crate::utils::regex::{
    fb2_rtf_mht_mhtml, fb3_epub, htm_html, md_fs_yml, изображение_расширение_с_точкой,
};
use crate::utils::zip::{
...
fn запись_если_есть_разница_архив(
    ...
    if !путь_сохранения.exists() {
        let сообщение_вывода: String =
            format!("☢️ Запись6: книга {}. Файл не существует. Запись", путь);
        if условие {
            вывод_сообщения_на_экран_и_вложение_в_ряд(
                сообщение_вывода,
                &mut сообщения,
    ...
    //read_utf8(&путь) //чтение файла в UTF-8
    match zip_архив_в_память(&путь,  HashMap::with_hasher(foldhash::fast::RandomState::default())) {
        Ok(успех) => успех,
        Err(ошибка) => {
            if sz_найти(&ошибка.to_string(), "Пустой файл") {
                вывод_сообщения_на_экран_и_вложение_в_ряд(
                    format!("☢️ Запись2:  книга {}. Пустое содержимого. Перезапись", путь),
                    &mut сообщения,
                );
                return Ok(true);
            } else {
