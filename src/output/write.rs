...
use crate::lib::{self, Куча_Словарь, Пути_Вывода, Содержимое_папок, Сообщения, Сообщения_для_книги, Счётчики_Словаря, Ячейка_замены_с_исключением};
...
fn utf8_to_windows1251(utf8_str: &str) -> Result<Vec<u8>, String> {
    ...
    if had_errors {
        ...
        return Err(format!(
            "Некоторые символы не могут быть представлены в Windows-1251"
        ));
    }
...
pub fn вывод_всех_счётчиков_замен_в_xls(
    ...
    // Add a worksheet to the workbook.
    счётчик_замен_в_словаре.однобуквенные=вывод_страницы_словаря_переносов(
        &mut словари,
        "Однобуквенные",
    ...
    .unwrap();
    счётчик_замен_в_словаре.двубуквенные=вывод_страницы_словаря_переносов(
        &mut словари,
        "Двубуквенные",
    ...
    .unwrap();
    счётчик_замен_в_словаре.трехбуквенные=вывод_страницы_словаря_переносов(
        &mut словари,
        "Трехбуквенные",
    ...
    .unwrap();
    счётчик_замен_в_словаре.многобуквенные=вывод_страницы_словаря_переносов(
        &mut словари,
        "Многобуквенные",
    ...
    .unwrap();
    счётчик_замен_в_словаре.целиковые=вывод_страницы_словаря_переносов(
        &mut словари,
        "Целиковые",
    ...
    .unwrap();
    счётчик_замен_в_словаре.исключения=вывод_страницы_словаря_переносов_исключения(
        &mut словари,
        "Исключения",
    ...
    let путь_сохранения: String =match указатель_переносов {
        ...
        1=> format!("{}Словари_переносов_длинное_тире.xlsx", пути_общие.вывод_словари),
        2=>format!("{}Словари_переносов_пробел_перед_тире.xlsx", пути_общие.вывод_словари),
        _ =>panic!(),
    };
        xlsx_сохранить_с_проверкой(&mut словари, &путь_сохранения);
    Ok(())
...
pub fn есть_ли_мусорные_расширения(строка:&String)->bool {
    lazy_static! {
        static ref исключения:Vec<String> = vec![
            ".epub".to_string(),
            ".fb2".to_string(),
              ".htm".to_string(),
            ".fb3".to_string(),
    ...
    return исключения.par_iter().any(|исключение| sz_найти(строка,исключение))
}
...
fn вывод_страницы_словаря(
    ...
    println!(
        "{имя_страницы}, количество. содержимое: {}| Куча: {} |, замен: {}",
        содержимое.len(),
        куча_словарь.len(),
        &всего_замен
    );
