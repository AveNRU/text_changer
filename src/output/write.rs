...
use crate::lib::{self, Имена_страниц, Куча_Словарь, Пути_Вывода, Словарь_разделителей, Содержимое_папок, Сообщения, Сообщения_для_книги, Счётчик_разделителей, Счётчики_Словаря, Ячейка_замены_с_исключением};
            ...
            let путь_вывода:String= {
                if !fb3_epub(&вложение.расширение) {
                  //  println!("пусто (до), а путь: {}",вложение.путь_откудаво);
                    let mut путь_до_папки: String = вложение.путь_откудаво
                        .replace(&пути_общие.книги, "");
...
fn вывод_страницы_словаря(
    ...
    let имя_страницы_для_вывода:String=match имя_страницы {
        ...
        Имена_страниц::составные_важные => "Составные_важные".to_string(),
        ...
        Имена_страниц::неизменные_длинные => "Неизменные_длинные".to_string(),
        Имена_страниц::неизменные_короткие => "Неизменные_короткие".to_string(),
    };
    let страница = книга.add_worksheet().set_name(имя_страницы_для_вывода).unwrap();
    ...
    for указатель in 0..содержимое.len() {
        ...
        страница
            .write((указатель + 1) as u32, 0, &содержимое[указатель].искомое_слово)
            .unwrap();
        страница
            .write((указатель + 1) as u32, 1, содержимое[указатель].re_образец.to_string())
            .unwrap();
        страница
            .write((указатель + 1) as u32, 2, содержимое[указатель].замена.to_string())
            .unwrap();
        страница
            .write(
                (указатель + 1) as u32,
...
pub fn вывод_страницы_словаря_разделителей_с_исключениями(
    ...
    страница.write(0, 0, "Изначальные слова").unwrap();
    страница.write(0, 1, "re_образец_для_поиска").unwrap();
    страница.write(0, 2, "re_образец_для_замены").unwrap();
    страница.write(0, 3, "замена").unwrap();
    страница.write(0, 4, "Количество случаев").unwrap();
    страница.write(0, 5, "Строка").unwrap();
    страница.write(0, 6, "Исключение").unwrap();
    //worksheet.write(0, 5, "Ток потребления").unwrap();
    //worksheet.write(0, 6, "Цепь земли (по умолчанию)").unwrap();
    ...
    for (указатель,ячейка) in содержимое.iter().enumerate() {
        ...
        всего_замен += счётчик[указатель].load(Ordering::Relaxed);
        страница
            .write((указатель + 1) as u32, 0, &ячейка.искомое_слово)
            .unwrap();
        страница
            .write((указатель + 1) as u32, 1, ячейка.re_образец_для_поиска.to_string())
            .unwrap();
        страница
            .write((указатель + 1) as u32, 2, ячейка.re_образец_для_замены.to_string())
            .unwrap();
        страница
            .write((указатель + 1) as u32, 3, ячейка.замена.to_string())
            .unwrap();
        
        //
        let mut указатель_столбца:u16=6;
        for указатель_исключения in ячейка.re_исключение.iter() {
            страница
                .write((указатель + 1) as u32, указатель_столбца, указатель_исключения.to_string())
                .unwrap();
            указатель_столбца+=1;
        }
...
fn вывод_страницы_словаря_переносов_исключения(
    ...
    for (указатель,ячейка) in содержимое.iter().enumerate() {
        ...
        страница
            ...
            .unwrap();
        страница
            .write((указатель + 1) as u32, 2, ячейка.замена.to_string())
            .unwrap();
        //
