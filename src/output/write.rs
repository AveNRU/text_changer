...
pub fn вывод_всех_счётчиков_замен_в_xls(
    ...
    счётчик_замен_в_словаре.исключения=вывод_страницы_словаря_переносов_исключения(
        ...
        .unwrap();
    let путь_сохранения: String = {if указатель_переносов ==0 {
        format!("{}Словари_переносов_обычные.xlsx", пути_общие.вывод_словари)
    } else {
        format!("{}Словари_переносов_изменённые.xlsx", пути_общие.вывод_словари)}
    };
    if счётчик_замен_в_словаре.подсчёт()>0 {
        xlsx_сохранить_с_проверкой(&mut словари, &путь_сохранения)
    }
