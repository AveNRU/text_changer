...
pub fn сохранить_книги(
    ...
    for i in 0..стопки_книг.len() {
        ...
        сообщения.запись_и_чтение[i].имя_книги=format!("{}.{}",стопки_книг[i].название_книги,стопки_книг[i].расширение);
        //путь до вывода
        ...
        if fb2_rtf_mhtml(&стопки_книг[i].путь) || md_fs_yml(&стопки_книг[i].путь)
        {
            ...
            for гл_указатель in 0..стопки_книг[i].вложения.len() {
                ...
                match запись_если_есть_разница(
                    ...
                    &mut сообщения.запись_и_чтение[i].сообщения,
                    true, //вывод на экран
                ) {
...
fn запись_если_есть_разница(
    ...
    if сравнение_двух_рядов_построчно(&новый_ряд_строк,&существующий_файл_ряд_строк){
