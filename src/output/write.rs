...
use crate::utils::zip::{VirtualFs, pack_zip_from_memory, zip_архив_в_память};
use encoding_rs::{
    WINDOWS_1251,
    //    DecoderResult
};
            ...
            for k in 0..стопки_книг[i].вложения.len() {
                ...
                //если это не картинки
                    ...
                    for k2 in 0..стопки_книг[i].вложения[k].содержимое.len()
                    {
                        ...
                        содержимое_байты.extend(стопки_книг[i].вложения[k].содержимое[k2].as_bytes());
...
pub fn xlsx_сохранить_с_проверкой(
    ...
    let условие = if путь.exists() {
        ...
        {
            println!(
                "XLSX файл: {} полностью совпадает с существующим XLSX файлом. Отаз от перезаписи.",
                путь_сохранения
            );
            true
        } else {
...
fn запись_если_есть_разница_архив(
    ...
    let архив: foldhash::HashMap<String, Vec<u8>> =
        foldhash::HashMap::with_hasher(foldhash::fast::RandomState::default());
    let mut книга_из_файла: VirtualFs = архив;
    
  

    //прочитать_содержимое_построчно(&пути_до_книг[i])
    //read_utf8(&путь) //чтение файла в UTF-8
    zip_архив_в_память(&путь, &mut книга_из_файла).unwrap();
    if книга_из_файла.len()==0 {
        вывод_сообщения_на_экран_и_вложение_в_ряд(
            format!(
                "Запись2:  книга {}. Не соответствие содержимого. Перезапись",
                путь
            ),
            &mut сообщения,
        );
    ...
    if озу_книга_куча.len()!=книга_из_файла.len() {
        вывод_сообщения_на_экран_и_вложение_в_ряд(
            format!("Длина в ОЗУ: {} не равна длине в файле: {}",озу_книга_куча.len(),книга_из_файла.len()),
            &mut сообщения,
        )
    }
    let mut счётчи_соответствий=0;
        ...
        for (имя_2,содержимое_2) in книга_из_файла.iter() {
            if имя.as_str()==имя_2.as_str() {
                if содержимое.as_slice()!=содержимое_2.as_slice() {
                    println!("Файл: {имя}. Содержимое не равно Vec<u8>")
                } else {счётчи_соответствий+=1;}
            }
        }
    }
    if счётчи_соответствий==озу_книга_куча.len() {
        println!("файлы схожи полностью")
    }
    if *озу_книга_куча == книга_из_файла {
        let сообщение_вывода = format!(
            "Запись8: книга {}. Полное совпадение. Отказ от перезаписи",
            путь
        );
        if условие {
            вывод_сообщения_на_экран_и_вложение_в_ряд(
        ...
        if условие {
            вывод_сообщения_на_экран_и_вложение_в_ряд(
                format!(
                    "Запись3:  книга {}. Не соответствие содержимого. Перезапись",
                    путь
                ),
                &mut сообщения,
            )
            ...
            вложить_строку_в_ряд_с_проверкой(
                &mut сообщения,
                &format!(
                    "Запись4:  книга {}. Не соответствие содержимого. Перезапись",
                    путь
                ),
            )
        }
