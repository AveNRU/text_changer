...
pub fn сохранить_книги(
   ...
   // let стопки_сообщений:Vec<Сообщения_для_книги>=
    стопки_книг.par_iter().enumerate().flat_map(|(i, книга)| {
        ...
        //если это архивное разрешение
            ...
            let вторичная_fs_в_озу: HashMap<String, Vec<u8>> =
                стопки_книг[i].вложения.par_iter().enumerate().flat_map(|(k,вложение)|{
...
fn запись_если_есть_разница(
    ...
    if существующий_файл_ряд_строк.len()==0 {
        let сообщение_вывода = format!(
            "Запись1:  файл {}. Пустой. Перезапись",
            путь
        );
        if условие {
            вывод_сообщения_на_экран_и_вложение_в_ряд(
