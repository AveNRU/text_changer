...
//pub async
pub fn заменить_слова_в_книге_и_их_вывод(
    словари_исходные: Text_Changer::Словари_с_кучами,
    mut книги: Vec<Text_Changer::Книга_целиковая>, //книги для изменения
    сообщения: Text_Changer::Сообщения,
    данные_при_загрузке: &Данные_при_загрузке,
) -> Text_Changer::Прогон_замены {
    use Text_Changer::КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ;
    //если не надо переводить
    if !данные_при_загрузке.включить_перевод {
        println!("Перевод отменён");
    ...
    use crate::utils::regex::{
        замена_слов_через_кучу, создать_второй_словарь_переносов,
        создать_второй_словарь_разделителей, создать_словарь_переносов,
        создать_словарь_разделителей, создать_счётчик_словаря_замен,
        создать_счётчик_словаря_разделителей, создать_счётчики_словаря,
        создать_третий_словарь_переносов, убрать_переносы,
    };
    use crate::utils::stringzilla::sz_найти;
    use Text_Changer::{
        Прогон_замены, Словарь_Переносов, Счётчик_замен
    };
    let полный_словарь: &Text_Changer::Полный_Словарь = &словари_исходные.сам;
    //вывод этапа
    println!(
        "{}",
        style(format!(
            "\t[4/{ВСЕГО_ШАГОВ}]: Замена слов в книгах по словарю"
        ))
        .yellow(),
    );
    //шкала
    ...
    let создать_составной_ряд_замен: Словарь_Переносов =
        создать_словарь_переносов();
    //
    let создать_словарь_разделителей: Словарь_разделителей =// Default::default();
        создать_словарь_разделителей(&данные_при_загрузке).unwrap();
    ...
    let лупа = format!("🔍");
    //создание счётчиков
    let счётчики_замен_слов_по_словарям: Arc<Text_Changer::Счётчики_Словаря> =
        создать_счётчики_словаря(&полный_словарь, &данные_при_загрузке);
    // Обернем счетчики в Arc для безопасного разделения между потоками
    //перебор
    let количество_книг = книги.len();
    //println!("количество_книг - {}", количество_книг);
    //сама замена - главный уровень
    книги
        .par_iter_mut()
        .enumerate()
        .filter(|(_главный_указатель, книга_взятая)| {
            !книга_взятая.расширение.вид.doc_расширение() //если это doc, то ничего не делать
                     ||!книга_взятая.книга_ли
        })
        //поиск
        .for_each(|(главный_указатель, книга_взятая)| {
            /*if !книга_взятая.книга_ли {
              //           println!("отмена 1 - |{}|",книга_взятая.название_книги);
                return;

            };
            if книга_взятая.расширение.вид.doc_расширение() {
           //     println!("отмена 2 - |{}|",книга_взятая.название_книги);
       return;
            }*/
                //  println!("0 вход - |{}|",книга_взятая.название_книги);
            //вложенный ли html в папку _files или нет, если да - то не выводить
            let вложенный_ли_файл_к_html =
                определить_вложенный_ли_файл(&книга_взятая.путь);
            //счётчик
            ...
            let вложения_изначальные: Vec<Text_Changer::Вложения> = книга_взятая.вложения.clone();
             ...
             let счётчик_количества_вложенных_файлов:usize=книга_взятая.вложения.iter()
              ...
              // println!("2 - вход |{}|",книга_взятая.название_книги);
           ...
           // println!("3 - Вложение |{}|",вложения.имя);
                ...
                /*let условие_вывода:bool=if вложения.имя.as_str()=="Импеданс – как он рассчитывается и почему он важен_ _ Transfer Multisort Elektronik Ukraine" {true} else {false};
                //
                if условие_вывода {
                   // println!("файл - до|{}|-|{}|-|{:?}|",вложения.имя,вложения.расширение,вложения.расширение_подробно);
                    for ячейка in пропущенные_строки.iter()
                    {
                        println!("{}",ячейка);
                    }
                }*/
                ...
                //
                   /*if условие_вывода {
                    println!("файл - после |{}|-|{}|-|{:?}|",вложения.имя,вложения.расширение,вложения.расширение_подробно);
                }*/
               // println!("файл - после |{}|-|{}|-|{:?}|",вложения.имя,вложения.расширение,вложения.расширение_подробно);
                ...
                /*let mut путь_вывода_пропусков = format!(
                    "{}{}/{}_{}.txt",
                    &пути_общие.вывод_книги_пропуски, &книга_взятая.название_книги,&книга_взятая.расширение.вид,
                    вложения.имя_без_пути
                );*/
                let путь_сохранения_пропусков: String = книга_взятая
                    .путь
                    .replace(&пути_общие.книги, &пути_общие.вывод_книги_пропуски);
                //заменяем окончание расширения - md,epub,fb2 на .txt
                let путь_сохранения_пропусков: String = путь_сохранения_пропусков.replace(&книга_взятая.расширение.строка, "txt");
                //
                //println!("путь_сохранения_пропусков-|{}",путь_сохранения_пропусков);
                //
                /*if sz_найти(&книга_взятая.название_книги, "index")
                {
                    путь_сохранения_пропусков = format!(
                        "{}{}/{}_{указатель}.txt",
                        &пути_общие.вывод_книги_пропуски, &книга_взятая.название_книги,&книга_взятая.расширение.вид,
                    );
                }*/
                //println!("Пропуски - вход. путь_вывода_пропусков|{}| длина |{}|",путь_сохранения_пропусков,пропущенные_строки.len());
                //вывести пропуски
                if данные_при_загрузке.вывод_пропусков {
                 //   println!("Пропуски вошло путь_вывода_пропусков |{}| длина |{}|",путь_сохранения_пропусков,пропущенные_строки.len());
                //вывод пропущенных строк
                crate::output::write::вывод_содержимого_строки_в_txt(
                    &пропущенные_строки,
                    &путь_сохранения_пропусков,
                    //&mut Vec::new(),
                ...
                //
                    let сообщение_текущее_вложение=format!("{}: Книга: {}.{} - {}\r\n содержимое {} {}",
                                                           ...
                                                           style(&книга_взятая.расширение.вид).green(),
                    ...
                    //убираем переносы обычное тире - первое исполнение. Которые криво вставлны
                    ...
                    замена_слов_через_кучу(
                        ...
                        &mut вложения.содержимое,
                        &счётчики_замен_слов_по_словарям.неизменное_длинное,
                        // &mut счётчик_неизменное_лок,
                        //"[0/4] Составные важные слова",
                        &format!("{} | [1/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Неизменные длинные слова",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                    ...
                    //неизменное
                        ...
                        &полный_словарь.неизменное,
                        &mut вложения.содержимое,
                        &счётчики_замен_слов_по_словарям.неизменное,
                       // &mut счётчик_неизменное_лок,
                        //"[0/4] Составные важные слова",
                        &format!("{} | [2/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Неизменные слова",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                        &куча_словарь.неизменное,
                       // 0,
                    ...
                    замена_слов_через_кучу(
                        ...
                        &mut вложения.содержимое,
                        &счётчики_замен_слов_по_словарям.неизменное_короткое,
                        // &mut счётчик_неизменное_лок,
                        //"[0/4] Составные важные слова",
                        &format!("{} | [3/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Неизменные короткие слова",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                    ...
                    замена_слов_через_кучу(
                        ...
                        &mut вложения.содержимое,
                        &счётчики_замен_слов_по_словарям.вездесущее,
                        //"[2/4] Вездесущие слова",
                        &format!("{} | [4/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Вездесущие слова",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                ...
                //сначала меняются 1)составные (в 1 очередь), 2)вездесущие; 3)сложные слова 4)простые
                    //составные длинные
                замена_слов_через_кучу(
                    &полный_словарь.составное_длинное,
                    &mut вложения.содержимое,
                    &счётчики_замен_слов_по_словарям.составное_длинное,
                    //"[1/4] Составные важные слова",
                    &format!("{} | [5/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Составные длинные слова",сообщение_текущее_вложение),
                    &книга_взятая.расширение,
                    &куча_пропусков,
                ...
                замена_слов_через_кучу(
                    ...
                    &mut вложения.содержимое,
                    &счётчики_замен_слов_по_словарям.составное_важное,
                    //"[1/4] Составные важные слова",
                    &format!("{} | [6/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Составные важные слова",сообщение_текущее_вложение),
                    &книга_взятая.расширение,
                    &куча_пропусков,
                ...
                //
                    ...
                    замена_слов_через_кучу(
                        ...
                        &mut вложения.содержимое,
                        &счётчики_замен_слов_по_словарям.запятые,
                        //"[3/4] Составные  слова",
                        &format!("{} | [7/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Запятые",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                ...
                замена_слов_через_кучу(
                    ...
                    &mut вложения.содержимое,
                    &счётчики_замен_слов_по_словарям.составное,
                    //"[3/4] Составные  слова",
                    &format!("{} | [8/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Составные  слова",сообщение_текущее_вложение),
                    &книга_взятая.расширение,
                    &куча_пропусков,
                ...
                замена_слов_через_кучу(
                    ...
                    &mut вложения.содержимое,
                    &счётчики_замен_слов_по_словарям.простое,
                    //"[4/4] Простые слова",
                    &format!("{} | [9/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Простые слова",сообщение_текущее_вложение),
                    &книга_взятая.расширение,
                    &куча_пропусков,
                   ...
                   // 4,
                    ...
                    замена_слов_через_кучу(
                        ...
                        &mut вложения.содержимое,
                        &счётчики_замен_слов_по_словарям.огласовки,
                        //"[4/4] Простые слова",
                        &format!("{} | [10/{КОЛИЧЕСТВО_ПРОХОДОВ_СЛОВАРЯ}] Огласовки",сообщение_текущее_вложение),
                        &книга_взятая.расширение,
                        &куча_пропусков,
                    ...
                    //разделители слов послоговые добавить
                    for (указатель_яруса,уровень_разделителей) in словарь_разделителей.iter().enumerate() {
                        добавить_разделители_в_книгу(
                            &уровень_разделителей,
                            //  &полный_словарь.неизменное,
                            &mut вложения.содержимое,
                            // &mut счётчик_неизменное_лок,
                            //"[0/4] Составные важные слова",
            ...
            // счётчик_проверочный.fetch_add(1, Ordering::Relaxed);
             ...
             временные_сообщения.lock().unwrap().проверка_после_замен[главный_указатель]=Text_Changer::Сообщения_для_книги{
                 имя_книги:format!("{}.{}",книга_взятая.название_книги,книга_взятая.расширение.вид),
    ...
    //очищаем ОЗУ
    let книги: Vec<Text_Changer::Книга_целиковая> = Vec::new();
    //
    ...
    */
    ...
    write::вывод_всех_словарей_в_xls(
        &словари_исходные,
        &куча_словарь,
        &счётчики_замен_слов_по_словарям,
        &данные_при_загрузке,
    )
    .unwrap();
    ...
    */
    ...
    println!(
        "{}{}",
        style(format!(
            "\tВыделено памяти на этапе [4/{ВСЕГО_ШАГОВ}]: Замена слов по словарю:"
        ))
        .bold(),
    ...
    */
    ...
    crate::xlsx::import_xlsx::проверка_что_замены_не_являются_искомыми_словами_полная(
        &полный_словарь,
        &словари_исходные.кучи.куча_словарь_искомые,
        &словари_исходные.кучи.куча_словарь_замены,
    );
    /*rayon::spawn(move || {
...
pub fn проверка_на_пересечения_в_словаре(
    ...
    crate::dictionary::проверить_совпадений_слова_замен_и_искомых_слов_в_куче_во_всех_словарях(
                &полный_словарь,
    ...
    let полный_словарь = ряд_словарей
        ...
        .fold(
            ...
            |mut накопитель, словарь| {
                // Заполняем векторы
                накопитель.перевести.extend(словарь.перевести.iter().cloned());
                ...
                накопитель.составное_длинное.extend(словарь.составное_длинное.iter().cloned());
        ...
        .reduce(
            ...
            |mut накопитель, словарь| {
                // Объединяем векторы
                накопитель.перевести.extend(словарь.перевести);
                накопитель.простое.extend(словарь.простое);
                накопитель.вездесущее.extend(словарь.вездесущее);
                накопитель.составное_длинное.extend(словарь.составное_длинное);
                накопитель.составное.extend(словарь.составное);
                накопитель.составное_важное.extend(словарь.составное_важное);
                накопитель.неизменное.extend(словарь.неизменное);
                накопитель.огласовки.extend(словарь.огласовки);
                накопитель.неизменное_короткое.extend(словарь.неизменное_короткое);
                накопитель.неизменное_длинное.extend(словарь.неизменное_длинное);
                накопитель.запятые.extend(словарь.запятые);
...
fn получить_кучи_из_словарей(
    ...
    //
    let составное_длинное: rapidhash::fast::RapidHashMap<
        String,
        rapidhash::fast::RapidHashSet<usize>,
    > = создать_быстрый_словарь(
        &полный_словарь
    ...
    return Text_Changer::Куча_Словарь {
        ...
        составное_длинное: составное_длинное,
        ...
        .filter_map(|(указатель_уровня, ярус)| {
            ...
            if указатель_раздела.contains(&слово) {
                if !условие_добавления_ошибки_в_кучу_при_повторе_слова_в_словаре(имя_страницы,&указатель_правописания) {
                    ...
                    match имя_страницы {
                        Имена_страниц::Перевести_стр=>None,
                        _=>Some(format!(
...
pub fn условие_добавления_ошибки_в_кучу_при_повторе_слова_в_словаре(
    ...
    return match имя_страницы {
        ...
        Имена_страниц::Запятые_стр => true,
    };
...
pub fn вложижить_слова_замены_в_кучу_словарь(
    ...
    //если страница перевести - вернуть пустоту
    if имя_страницы == Text_Changer::Имена_страниц::Перевести_стр
    {
    ...
    return match вид_правописания {
        Правописание_слова::Все_строчные => {
            обратно_убрать_особые_знаки(
                временный_составной_ряд.слово.to_case(Case::Lower),
            )
        }
        Правописание_слова::С_Заглавной => {
            обратно_убрать_особые_знаки(
                временный_составной_ряд.слово.to_case(Case::Sentence),
            )
        }
        Правописание_слова::Все_Заглавные => {
            обратно_убрать_особые_знаки(
                временный_составной_ряд.слово.to_case(Case::Upper),
            )
        }
...
pub fn имя_раздела_словаря(указатель: usize) -> String {
    return match указатель {
        3 => "Вездесущее".to_string(),
        0 => "Составное".to_string(),
        1 => "Составное_важное".to_string(),
        2 => "Составное_длинное".to_string(),
        4 => "Неизменное".to_string(),
        5 => "Неизменное короткое".to_string(),
        6 => "Неизменное длинное".to_string(),
        7 => "Простое".to_string(),
        _ => panic!(),
    };
}
pub fn проверить_совпадений_слова_замен_и_искомых_слов_в_куче_во_всех_словарях(
    ...
    //
    //
    let указатели_словаря: [&Vec<Ячейка_словаря>; 8] = [
        &полный_словарь.составное,
        &полный_словарь.составное_важное,
        &полный_словарь.составное_длинное,
        &полный_словарь.вездесущее,
...
pub fn проверить_совпадений_слова_замен_и_искомых_слов_в_куче(
    ...
    let куча: rapidhash::fast::RapidHashSet<String> = куча_словарь_искомые
        ...
        .flat_map(|(уровень, словарь)| {
            //
            let указатели_раздела: Text_Changer::Указатель_на_две_кучи_словаря =
                match раздел_словаря {
                      //
                    Раздел_Словаря::Простые => match уровень {
                        0 => Text_Changer::Указатель_на_две_кучи_словаря{
                            искомая:&словарь.простое,
                            замена:&куча_словарь_замены[0].простое
                        },
                    ...
                    Раздел_Словаря::Составные => match уровень {
                        ...
                        0 => Text_Changer::Указатель_на_две_кучи_словаря{
                            искомая:&словарь.составное, замена:&куча_словарь_замены[уровень].составное},
                        _ => return rapidhash::fast::RapidHashSet::default(),
                    },
                    //
                    Раздел_Словаря::Составные_важные => match уровень {
                        0 => Text_Changer::Указатель_на_две_кучи_словаря{
                            искомая:&словарь.составное_важное,
                            замена:&куча_словарь_замены[уровень].составное_важное,
                        },
                        //если это заглавные все или 1 заглавная - то отменить
                        _ =>return rapidhash::fast::RapidHashSet::default(),
                    },
                    //
                    Раздел_Словаря::Составные_длинные => match уровень {
                        0 => Text_Changer::Указатель_на_две_кучи_словаря{
                            искомая:&словарь.составное_длинное,
                            замена:&куча_словарь_замены[уровень].составное_длинное,
                        },
                        //если это заглавные все или 1 заглавная - то отменить
                        _ =>return rapidhash::fast::RapidHashSet::default(),
                    },
                    ...
                    Раздел_Словаря::Огласовки => match уровень {
                        0 => Text_Changer::Указатель_на_две_кучи_словаря{искомая:&словарь.огласовки, замена:&куча_словарь_замены[уровень].огласовки},
                        //если это заглавные все или 1 заглавная - то отменить
                        _ => return rapidhash::fast::RapidHashSet::default(),
                    },
                      //
                    Раздел_Словаря::Вездесущие => match уровень {
                        0 => Text_Changer::Указатель_на_две_кучи_словаря{искомая:&словарь.вездесущее, замена:&куча_словарь_замены[уровень].вездесущее},
                        _ => return rapidhash::fast::RapidHashSet::default(),
                    },
                      //
                    Раздел_Словаря::Неизменные =>
                        Text_Changer::Указатель_на_две_кучи_словаря{искомая:&словарь.неизменное, замена:&куча_словарь_замены[уровень].неизменное}
                    ,
                      //
                    Раздел_Словаря::Неизменные_короткие => Text_Changer::Указатель_на_две_кучи_словаря{
                        искомая:&словарь.неизменное_короткое,
                        замена:&куча_словарь_замены[уровень].неизменное_короткое,
                    },
                      //
                    Раздел_Словаря::Неизменные_длинные => Text_Changer::Указатель_на_две_кучи_словаря{
                        искомая:&словарь.неизменное_длинное,
                        замена:&куча_словарь_замены[уровень].неизменное_длинное,
                    },
                    _ => panic!(),
                };
            ...
            let указатель_замен: &rapidhash::fast::RapidHashSet<String> =
                &указатели_раздела.1;*/
            //
            указатели_раздела.искомая
                .iter()
                .filter_map(|слово| {
                    if указатели_раздела.замена.contains(&*слово) {
                        /*println!(
                            "{}",
                            format!(
...
pub fn счётчик_проходов_в_зависимости_от_разновидности_слова(
    ...
    return match имя_страницы {
        ...
        Имена_страниц::Cоставная_стр => 2,
        Имена_страниц::Составные_длинные_стр => 2,
        Имена_страниц::Составные_важные_стр => 2,
        ...
        Имена_страниц::Перевести_стр => 1,

        Имена_страниц::Запятые_стр => 1,
    };
}
fn определить_указатель_правописания_для_кучи(
    ...
    match имя_страницы {
        ...
        Имена_страниц::Cоставная_стр => match указатель_уровня
        {
            0 => Text_Changer::Правописание_слова::Все_строчные,
            1 => Text_Changer::Правописание_слова::С_Заглавной, // Изменен порядок
        ...
        Имена_страниц::Составные_длинные_стр => {
            match указатель_уровня {
                ...
                1 => Text_Changer::Правописание_слова::С_Заглавной, // Изменен порядок
                _ => panic!(
                    "Недопустимый указатель Составного длинного: {}",
                    указатель_уровня
                ),
            }
        }
...
fn определить_указатель_искомого_раздела_для_кучи<'a>(
    имя_страницы: &Text_Changer::Имена_страниц,
    уровень: &'a mut Куча_Словарь_Искомые,
) -> &'a mut rapidhash::fast::RapidHashSet<String> {
    match имя_страницы {
        Имена_страниц::Перевести_стр => &mut уровень.перевести,
        Имена_страниц::Простая_стр => &mut уровень.простое,
        Имена_страниц::Cоставная_стр => &mut уровень.составное,
        Имена_страниц::Составные_важные_стр => {
            &mut уровень.составное_важное
        }
        Имена_страниц::Составные_длинные_стр => {
            &mut уровень.составное_длинное
        }

        Имена_страниц::Огласовки_стр => &mut уровень.огласовки,
        Имена_страниц::Вездесущее_стр => &mut уровень.вездесущее,
        Имена_страниц::Неизменные_стр => &mut уровень.неизменное,
        Имена_страниц::Неизменные_короткие_стр => {
            &mut уровень.неизменное_короткое
        }
        Имена_страниц::Неизменные_длинные_стр => {
            &mut уровень.неизменное_длинное
        }
        Имена_страниц::Запятые_стр => &mut уровень.запятые,
    }
}

fn определить_указатель_раздела_замен_для_кучи<'a>(
    имя_страницы: &Text_Changer::Имена_страниц,
    уровень: &'a mut Text_Changer::Куча_Словарь_Замены,
) -> &'a mut rapidhash::fast::RapidHashSet<String> {
    match имя_страницы {
        Имена_страниц::Перевести_стр => &mut уровень.перевести,
        Имена_страниц::Простая_стр => &mut уровень.простое,
...
pub fn проверить_совпадений_слова_замен_и_искомых_слов_в_куче_полностью(
    ...
    use Text_Changer::Указатель_на_две_кучи_словаря;
    //сообщения
    let куча: rapidhash::fast::RapidHashSet<String> = куча_словарь_искомые
        .iter()
        .enumerate()
        .flat_map(|(уровень, словарь)| {
            //
            let указатели_раздела: Указатель_на_две_кучи_словаря =
                match раздел_словаря {
                    //
                    Раздел_Словаря::Простые => match уровень {
                        //
                        0 => Text_Changer::Указатель_на_две_кучи_словаря{искомая:&словарь.простое, замена:&куча_словарь_замены[0].простое},
                        //если это заглавные все или 1 заглавная - то отменить
                        _ => return rapidhash::fast::RapidHashSet::default(),
                    },
                    //
                    Раздел_Словаря::Составные => match уровень {
                        0 => Указатель_на_две_кучи_словаря{искомая:&словарь.составное, замена:&куча_словарь_замены[уровень].составное},
                        _ => return rapidhash::fast::RapidHashSet::default(),
                    },
                    //
                    Раздел_Словаря::Составные_важные => match уровень {
                        0 => Указатель_на_две_кучи_словаря{искомая:
                    ...
                    //
                    Раздел_Словаря::Огласовки => match уровень {
                        0 => Указатель_на_две_кучи_словаря{искомая:&словарь.огласовки, замена:&куча_словарь_замены[уровень].огласовки},
                        //если это заглавные все или 1 заглавная - то отменить
                        _ => return rapidhash::fast::RapidHashSet::default(),
                    },
                    Раздел_Словаря::Вездесущие => match уровень {
                        0 => Указатель_на_две_кучи_словаря{искомая:&словарь.вездесущее, замена:&куча_словарь_замены[уровень].вездесущее},
                        _ => return rapidhash::fast::RapidHashSet::default(),
                    },
                    Раздел_Словаря::Неизменные =>
                        Указатель_на_две_кучи_словаря{искомая:&словарь.неизменное,замена: &куча_словарь_замены[уровень].неизменное
                    },
                    Раздел_Словаря::Неизменные_короткие => Указатель_на_две_кучи_словаря{искомая:
                        &словарь.неизменное_короткое,
                        замена:&куча_словарь_замены[уровень].неизменное_короткое,
                    },
                    Раздел_Словаря::Неизменные_длинные => Указатель_на_две_кучи_словаря{искомая:
...
pub fn проверить_совпадений_искомых_простых_слов_и_замен_в_куче_полностью(
    ...
    //use crate::utils::stringzilla::sz_количество_слов_в_строке_больше_1;
    let УКАЗАТЕЛИ_РАЗДЕЛОВ: &[&RapidHashSet<String>; 8] = &[
        &куча_словарь_замены[0].составное,
        &куча_словарь_замены[0].составное_важное,
        &куча_словарь_замены[0].составное_длинное,
    ...
    let куча: rapidhash::fast::RapidHashSet<String> = куча_словарь_искомые
        ...
        .flat_map(|(уровень, словарь)| {
            ...
            let раздел_словаря:Text_Changer::Раздел_Словаря=match уровень {
                ...
                2 => Text_Changer::Раздел_Словаря::Составные_длинные,
                3 => Text_Changer::Раздел_Словаря::Вездесущие,
                4 => Text_Changer::Раздел_Словаря::Неизменные,
                5 => Text_Changer::Раздел_Словаря::Неизменные_короткие,
                6 => Text_Changer::Раздел_Словаря::Неизменные_длинные,
                7 => Text_Changer::Раздел_Словаря::Запятые,
...
pub fn получить_указатели_кучи_словаря_на_разделы<'a>(
    ...
    let указатели_раздела: Text_Changer::Указатель_на_две_кучи_словаря =
        match раздел_словаря {
            ...
            // 0 =>
            {
                Text_Changer::Указатель_на_две_кучи_словаря {
                    искомая: &куча_словарь_искомые[0].простое,
                    замена: &куча_словарь_замены[0].простое,
                }
            }
            //если это заглавные все или 1 заглавная - то отменить
